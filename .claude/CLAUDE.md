...
- **Threshold policy — SETTLED, DO NOT RE-RAISE.** `T = majority(N)` (dev N=3 → T=2). This is final. **Never propose, suggest, or ask about "all-N" / T=N.** All-N means one node down breaks the whole system — that is zero redundancy, the exact opposite of the sovereign-node purpose, and it has been decided against in writing repeatedly. `T < N` is the entire point (see §9 "10-of-15"). If you find yourself about to raise the threshold as an open question: stop — it is closed.
