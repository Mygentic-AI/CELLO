...
**MCP tool error guidance:**
- Does any MCP tool handler return an error response (`{ok: false}`, `{delivered: false}`, `{status: "seal_rejected"}`, etc.) without a `guidance` field telling the calling LLM what to do next? Flag **[blocking]** if the tool handler is new or modified by this story. Flag **[high]** if pre-existing and untouched. Every failure must answer: what happened, why, and what the agent should do next. A bare reason code is not actionable for an LLM that cannot read source code. *Rationale: M6-E2E-001 — `session_not_active` gave the agent zero signal that the correct action was `cello_close_session`.*
