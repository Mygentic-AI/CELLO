...
**When the environment can't satisfy an AC's structural contract — stop and raise it.** ACs describe not just what should be produced but how — participant counts, communication topology, independence constraints, ordering, who talks to whom. If any of that structure cannot be realized with the infrastructure that exists right now, do not collapse the structure silently to something that produces the same output through a different shape. The correct action is to flag it as a blocker before writing any implementation code: state which AC cannot be satisfied, what is missing, and what the minimum viable approach would be if a deferral is accepted.

This applies to any structural simplification: routing everything through one node when the AC implies independent peer-to-peer exchange; using a single in-process instance when the AC requires separate processes; serializing steps the AC implies are concurrent; or substituting a relay for a direct connection. The result may look correct. The structural guarantee — the property the AC was written to protect — will not exist.

The same principle applies to implementation technology, and the failure mode here is subtle: the constraint is usually correctly specified somewhere — in CLAUDE.md or an earlier discussion log — but was not restated in the story you are reading. The story is self-contained enough to implement from, and you reach for the most obvious available tool. That tool satisfies the mechanism the story describes while violating a constraint the story never mentioned because it was inherited implicitly from an earlier document.

*The canonical example:* Postgres logical replication was correctly specified as the federation mechanism. The story did not mention that the transport must be cloud-agnostic. The implementer reached for VPC Peering — the obvious tool for connecting three private AWS subnets — without re-reading the April 8 document that established the multi-cloud constraint. The mechanism was right. The transport was wrong. All tests passed.

Before choosing how to instantiate any mechanism that involves inter-node communication, synchronization, coordination, or infrastructure provisioning, check each layer against CLAUDE.md independently — not just against the story. The mechanism may be correct while the transport or operational model beneath it violates a system-wide invariant. If it does, raise it before implementing.

The reviewer catches these deviations, but the coder must not create them without a conscious decision and a documented deferral in the story's `stubs` section.
