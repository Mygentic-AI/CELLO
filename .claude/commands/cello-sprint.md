...
The same principle applies to implementation technology. If a story implies that nodes are independent — sovereign, separately operable, cloud-agnostic — do not implement synchronization or coordination through a cloud-provider-specific service (managed replication, a vendor control plane, a shared AWS service) just because it is convenient and makes the tests pass. That implementation satisfies the observable behavior while creating an invisible coupling that violates the independence guarantee. Before choosing any infrastructure technology for synchronization, coordination, or state sharing, ask: *"Could each node using this be operated by a different organization on a different cloud provider without any shared account or vendor relationship?"* If no — and the story implies yes — that is a structural violation that must be raised before implementation proceeds.
