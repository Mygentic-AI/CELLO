...
|---|---|---|---|
| Identity key | Yes — in KeyProvider | No | BIP-39 seed phrase backup |
| Signing key (K_local) | Yes — in KeyProvider | No | Rotated independently; not backed up via seed phrase |
...
| ~~AC-57~~ | Connections | ~~Resolved~~ — AC-11 resolved that no signals are mandatory at the protocol level. Initiating agent chooses what to include; omitting signals increases decline likelihood but is permitted. The partial-disclosure question is therefore moot: there is no mandatory signal set to partially disclose. |
...
| ~~AC-81~~ | Recovery | ~~Resolved~~ — Succession package creation (5-step flow: fetch successor's identity_key, encrypt identity key private bytes, upload blob with signature) and decryption (6-step flow: successor decrypts with own identity_key, derives predecessor's key, signs migration) are fully specified in Part 1 §Succession package. |
