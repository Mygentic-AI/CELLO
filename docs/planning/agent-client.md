...
| ~~AC-81~~ | Recovery | ~~Resolved~~ — Succession package creation (5-step flow: fetch successor's identity_key, encrypt seed phrase, upload blob with signature) and decryption (6-step flow: successor decrypts with own identity_key, derives predecessor's key, signs migration) are fully specified in Part 1 §Succession package. |
