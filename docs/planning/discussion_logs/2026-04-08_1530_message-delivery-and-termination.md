...
- [[agent-client|CELLO Agent Client Requirements]] — Part 4 implements the delivery failure tree (Cases A–D), control leaves, session attestation, and the bilateral vs. notarized seal distinction; Part 3 covers the dual-path dispatch mechanics
- [[2026-04-17_1100_not-me-session-termination|"Not Me" Session Termination — Dual-Path Forced Abort]] — adds a forced ABORT path to the termination protocol: directory-initiated EMERGENCY_SESSION_ABORT and PEER_COMPROMISED_ABORT for the compromise case, distinct from the voluntary CLOSE/CLOSE-ACK flow designed here
- [[2026-04-18_1620_commerce-attestation-and-fraud-detection|Commerce Attestation and Fraud Detection]] — session close is the natural point where purchase attestations are generated and signed; the Merkle seal anchors the attestation record
- [[2026-04-19_2045_group-room-design|Group Room Design]] — extends the session termination protocol to group rooms: DISSOLVE (forced seal), LEAVE (per-participant departure with last_seen_seq), CHECKPOINT (periodic health pulse replacing per-session EXPIRE for active rooms), and ABSENT transition for inactive participants
- [[2026-04-24_1530_inference-billing-protocol|Inference Billing Protocol]] — adds ABORT-BILLING termination reason for inference sessions that breach cost caps or fail per-turn count verification
- [[2026-06-20_2220_tier5-recovery-substrate-disposition|Tier-5 Recovery Substrate Disposition]] — maps this log's delivery-failure tree (Cases A–D + the time dimension) to M7 DoD REC-3, confirming each branch has a home in MSG-001 + SESSION-004 + PERSIST-012 + DAEMON-003
