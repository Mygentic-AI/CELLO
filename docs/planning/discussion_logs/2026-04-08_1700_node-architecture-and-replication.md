...
- [[2026-04-15_1100_key-rotation-design|Key Rotation Design]] — proactive secret sharing (share refresh) for node recovery operates across the same node infrastructure; K_server_X shares stored per-node extend this architecture with per-agent key material
- [[2026-04-13_1400_meta-merkle-tree-design|Meta-Merkle Tree Design]] — distributed MMR construction builds on the federation checkpoint mechanism designed here; deterministic batch processing at checkpoint boundaries avoids real-time ordering consensus
- [[server-infrastructure|CELLO Server Infrastructure Requirements]] — the node architecture, replication strategy, FROST threshold setup, and WebSocket infrastructure designed here are directly reflected in the server infrastructure requirements
- [[2026-04-17_1400_directory-relay-architecture-reassessment|Directory/Relay Architecture Reassessment]] — extends this node architecture by giving relay nodes session-level Merkle engine responsibilities (hash relay, sequencing, tree building); relay nodes need analogous backup selection for session resilience
