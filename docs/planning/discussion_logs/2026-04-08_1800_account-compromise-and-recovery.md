...
- [[2026-04-13_1200_discovery-system-design|Discovery System Design]] — the Class 3 group chat Merkle non-repudiation feeds into the dispute resolution and arbitration designed here; same arbitration system handles both direct conversations and group rooms
- [[2026-04-14_0700_agent-succession-and-ownership-transfer|Agent Succession and Ownership Transfer]] — reuses the M-of-N recovery contacts, 48-hour waiting period, and tombstone infrastructure designed here; succession extends the tombstone model with a new `SUCCESSION_INITIATED` type and adds a designated successor role distinct from recovery contacts
- [[2026-04-15_1100_key-rotation-design|Key Rotation Design]] — K_local rotation is the agent-side complement to compromise recovery; tombstone triggers K_server_X invalidation; K_local rotation with directory nudges is the proactive defence before compromise occurs
- [[2026-04-16_1400_companion-device-architecture|Companion Device Architecture]] — companion device provides the "Not Me" emergency revocation surface; push notifications for compromise alerts use the same out-of-band channel designed here
- [[frontend|CELLO Frontend Requirements]] — social recovery flow UI (M-of-N contacts, 48-hour countdown, voucher accountability), tombstone display, and compromise detection views sourced from this log
- [[2026-04-17_1100_not-me-session-termination|"Not Me" Session Termination — Dual-Path Forced Abort]] — operationalises the tombstone effect "all active sessions seal on any tombstone" with a concrete dual-path mechanism; resolves how existing P2P sessions are closed when K_server is burned
- [[server-infrastructure|CELLO Server Infrastructure Requirements]] — social recovery mechanics, tombstone types, voucher accountability, arbitration system, and dispute resolution (G-23 through G-28) are the server-side implementations of the mechanisms designed here