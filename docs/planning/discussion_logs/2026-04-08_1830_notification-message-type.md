...
- [[2026-04-14_1100_cello-mcp-server-tool-surface|CELLO MCP Server Tool Surface]] — `cello_poll_notifications` surfaces the notification primitive designed here; notification types include `connection_request`, `endorsement_received`, `security_block`, and `system` events
- [[2026-04-14_0700_agent-succession-and-ownership-transfer|Agent Succession and Ownership Transfer]] — the `succession_claim_filed` notification (sent to owner, recovery contacts, and all connected agents when a succession claim is initiated) uses the fire-and-forget primitive designed here
- [[2026-04-14_1300_connection-request-flow-and-trust-relay|Connection Request Flow — Trust Data Relay and Selective Disclosure]] — introduction notifications are the fallback path when the pre-connection endorsement check in that log's flow is insufficient
- [[frontend|CELLO Frontend Requirements]] — notification centre UI, per-type filtering controls, and the formal notification type registry (introduction, order-update, alert, promotional, system, tombstone, trust event, recovery event, attestation dispute) sourced from this log
- [[server-infrastructure|CELLO Server Infrastructure Requirements]] — the full notification type registry is specified there; rate limits by trust tier (G-31), bio rate limit (G-29), greeting rate limit (G-30) are resolved there; TRUST_SIGNAL_PICKUP_PENDING added in the pickup queue design
- [[2026-04-18_1407_push-publish-subscription-model|Push-Publish Subscription Model]] — push-publish deliveries are structured notifications; this log's fire-and-forget primitive is the foundation for scheduled micropublishing content delivery
