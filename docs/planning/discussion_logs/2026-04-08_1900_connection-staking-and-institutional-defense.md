...
- [[cello-initial-design|CELLO Design Document]] — Connection Staking section in Step 6; the Gate Pyramid is reproduced there
...
- [[2026-04-14_1100_cello-mcp-server-tool-surface|CELLO MCP Server Tool Surface]] — `cello_accept_connection` and `cello_decline_connection` implement the connection gate; `cello_manage_policy` configures the signal requirements the gate pyramid evaluates
- [[2026-04-14_1300_connection-request-flow-and-trust-relay|Connection Request Flow — Trust Data Relay and Selective Disclosure]] — the gate pyramid evaluates the trust data whose relay mechanism is defined there; staking requirements are checked during Phase 2 policy evaluation
- [[server-infrastructure|CELLO Server Infrastructure Requirements]] — staking hooks specified as architecturally present from day one (all defaults zero); bonded rate limit tier (G-31) noted as future extension pending G-36 financial infrastructure; institutional verification (G-33) deferred to BD/product
- [[2026-04-18_1357_connection-bond-usage-and-policy|Connection Bond Usage and Policy]] — extends this log's staking design with a two-mode bond model (voluntary trust signal vs. defensive receiver requirement) and adds mandatory intent declaration and policy-first connection flow
- [[2026-04-18_1620_commerce-attestation-and-fraud-detection|Commerce Attestation and Fraud Detection]] — economic stake complements behavioral fraud detection; bonds deter the money-transfer attack pattern analyzed there
- [[2026-04-19_2045_group-room-design|Group Room Design]] — extends the gate pyramid principle to room-level traffic: relay pre-room gates and per-room manifest enforcement are a direct application of the layered defense model designed here
