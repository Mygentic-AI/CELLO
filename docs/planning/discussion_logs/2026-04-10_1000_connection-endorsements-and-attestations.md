...
- [[2026-04-14_1000_contact-alias-design|Contact Alias Design]] — alias-specific connection policies use the same signal requirement model established here; endorsement requirements can be set per alias independently of the agent's global connection policy
- [[2026-04-14_1100_cello-mcp-server-tool-surface|CELLO MCP Server Tool Surface]] — endorsement signals surfaced via `cello_verify`; `SignalRequirementPolicy` uses endorsement count as a connection gate criterion; endorsement management tools noted as a gap in §9
- [[2026-04-14_1300_connection-request-flow-and-trust-relay|Connection Request Flow — Trust Data Relay and Selective Disclosure]] — endorsement details are classified as discretionary signals in the mandatory/discretionary framework; Alice chooses whether to reveal specific endorsers
- [[frontend|CELLO Frontend Requirements]] — endorsement management UI (give/receive/revoke), endorsement signal display in trust cards, and per-alias endorsement policy configuration sourced from this log
