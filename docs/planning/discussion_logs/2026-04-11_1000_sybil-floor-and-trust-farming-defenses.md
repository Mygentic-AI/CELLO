...
- [[server-infrastructure|CELLO Server Infrastructure Requirements]] — conductance score (G-21 resolved: checkpoint-computed, internal only), endorsement rate limit (G-17: 10/month), recovery contact floor for device attestation (G-24), and bonded rate limit tier (G-31) are specified there
- [[2026-04-18_1357_connection-bond-usage-and-policy|Connection Bond Usage and Policy]] — formalizes the two-mode bond model; the voluntary trust signal bond and receiver-mandated bond are the economic Sybil defense layer this log designs for
- [[2026-06-07_1221_sybil-confirm-shortcut-audit|Sybil Defense Audit — CONFIRM Shortcut and Per-Phone Agent Cap]] — verifies the M6 CONFIRM flow against this design: multi-agent-per-phone is confirmed intentional and account-linked; identifies missing per-account agent cap as the one unimplemented closure; proposes minimum story
