...
- [[cello-initial-design|CELLO Design Document]] — Step 3 (directory authentication), Step 6 (session establishment, dual-path hash relay), Step 7 (Merkle tree, prev_root, CLOSE/CLOSE-ACK), Federation section (federated node model)
...
- [[2026-04-17_1400_directory-relay-architecture-reassessment|Directory/Relay Architecture Reassessment]] — circuit relay v2 configuration requirements for long-lived sessions (unlimited reservation duration); relay nodes take over hash relay and sequencing during sessions using the transport infrastructure designed here
