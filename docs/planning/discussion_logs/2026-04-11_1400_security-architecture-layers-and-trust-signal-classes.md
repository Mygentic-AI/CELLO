...
- [[2026-05-16_1130_security-layer-improvements-from-production-reference|Security Layer Improvements from Production Reference Analysis]] — six concrete improvements derived from studying production code: RE2 engine for Layer 1 pattern matching (closes ReDoS), entropy scoring for Step 8, three missing PII types in Layer 4, honey tokens as active deception, policy change approval gate (extends Layer 5a), and production-concrete audit log streaming
- [[2026-05-21_1456_identity-as-governance-foundation|Identity as the Foundation of Governance]] — applies the signal class taxonomy to the governance question; identity-first design is what lets the four security layers become per-entity policies rather than one-size-fits-all rules
- [[2026-05-28_1000_security-layer-v3-extensibility-and-split-gateway|Security Layer V3 — Extensibility, Hook Architecture, and Split Gateway]] — V3 redesign of Layer 3 (client protection) into a separate gateway process with two-tier extensibility, directory-backed hash chain, and enterprise split-deployment; the four-layer system model here is unchanged but the Layer 3 implementation home moves to the gateway
