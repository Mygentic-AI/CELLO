...
- [[2026-04-14_1300_connection-request-flow-and-trust-relay|Connection Request Flow — Trust Data Relay and Selective Disclosure]] — the definitive flow that the `connection_requests` schema here implements: `outcome`, `rejection_reason`, `via_alias_id`, `escalation_expires_at`, mandatory vs. discretionary signal classification, and the ephemeral data policy
- [[2026-04-15_0900_session-level-frost-signing|Session-Level FROST Signing]] — no schema changes required; signing mechanism is above the persistence layer. Updated `initial_fallback_pubkey_hash` description to reflect K_local as the standard per-message signing key
- [[2026-04-15_1100_key-rotation-design|Key Rotation Design]] — K_server_X shares are stored per-agent in the node database with envelope encryption; proactive share refresh is the recovery mechanism when a node loses its share database
- [[2026-04-14_1500_deprecate-trust-seeders-and-trustrank|Deprecate Trust Seeders and TrustRank]] — removes the Trust Seeder registry, vouching, accountability, and lockout tables defined in this schema; the seeder role provides no evaluative capability the signal-based model doesn't already have
- [[2026-04-16_1400_companion-device-architecture|Companion Device Architecture]] — extends client-side persistence with local-only entry types (human_injected, human_requested) and the `merkle_leaf_hash` discriminator for protocol vs. non-protocol log entries
- [[frontend|CELLO Frontend Requirements]] — the persistence schema defines what state the frontend can display: session records, attestation rows, alias tables, notification events, endorsement hashes, and trust signal columns
- [[server-infrastructure|CELLO Server Infrastructure Requirements]] — the PostgreSQL schema, append-only RLS enforcement, and federation replication strategy defined here are the basis of the directory server's persistence infrastructure requirements
- [[agent-client|CELLO Agent Client Requirements]] — Part 8 implements the client-side persistence layer: SQLCipher database, conversation log entry types with `merkle_leaf_hash` discriminator, local-only tables (contact list, alias records, recovery contact obligations, human injection log), and encrypted cloud backup
- [[2026-04-18_1454_merchant-crm-data-stash-and-free-samples|Merchant CRM Data Stash and Free Sample Tracking]] — the stash records (per-contact JSON blobs keyed by identity_key) need a schema entry in the client-side SQLCipher persistence layer defined here
- [[2026-04-18_1620_commerce-attestation-and-fraud-detection|Commerce Attestation and Fraud Detection]] — attestation hashes need schema entries; raw text attestations above the dollar threshold require a conditional storage model extending this persistence design
- [[2026-04-19_2045_group-room-design|Group Room Design]] — requires schema additions: room ownership table, violation/mute tracking per participant, JOIN/LEAVE/CHECKPOINT/DISSOLVE/AUTO_MUTE/KICK control leaf types, per-room budget cap records, and banned_agents list
