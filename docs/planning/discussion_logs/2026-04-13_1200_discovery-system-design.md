...
- [[2026-04-14_1100_cello-mcp-server-tool-surface|CELLO MCP Server Tool Surface]] — implements discovery tools: `cello_search` (unified three-class search), `cello_create_listing`, `cello_update_listing`, `cello_renew_listing`, `cello_retire_listing`, and `cello_create_room`
- [[2026-04-14_1300_connection-request-flow-and-trust-relay|Connection Request Flow — Trust Data Relay and Selective Disclosure]] — discovery is the step immediately preceding the connection request flow; Alice finds Bob's public profile, then the trust data relay begins
- [[2026-04-14_1500_deprecate-trust-seeders-and-trustrank|Deprecate Trust Seeders and TrustRank]] — the three-class discovery system (listings, group rooms, open policies) is the organic cold-start path that makes the Trust Seeder role unnecessary
- [[frontend|CELLO Frontend Requirements]] — discovery UI across all three classes (agent directory search, bulletin board listings, group room browser), provisional period badge, and rate-limit display sourced from this log
- [[server-infrastructure|CELLO Server Infrastructure Requirements]] — C-6 (two-tier discovery access) resolved here: public browse is unauthenticated; protocol operations require authenticated FROST session; bio rate limit (G-29), greeting rate limit (G-30) specified there
- [[2026-04-18_1620_commerce-attestation-and-fraud-detection|Commerce Attestation and Fraud Detection]] — seller track record (distinct buyer count, delivery history) surfaces in discovery and is a visible trust signal before a transaction is attempted; thin track record is a fraud indicator
- [[2026-04-19_2045_group-room-design|Group Room Design]] — replaces the `room_type` enum with a two-flag model (`discoverable`/`private`); adds selective rooms (discoverable + petition required) as a third configuration beyond the original open/invite-only pair
- [[2026-06-25_2109_agent-identity-lifecycle-discovery|Agent Identity, Lifecycle & Discovery Model]] — reframes discovery as a layered model (introductions/petnames → oblivious exact-match handle → opt-in public bio/listing) under an anti-surveillance lens, tightening the "public browse" agent directory here (prior-art synthesis: petnames, Bluesky, Signal oblivious lookup); unifies "whitelisting = known = petname" with the M10 connection policy
