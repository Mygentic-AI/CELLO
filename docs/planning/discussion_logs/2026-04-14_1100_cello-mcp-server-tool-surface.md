...
- [[agent-client|CELLO Agent Client Requirements]] — the client is the runtime host for the MCP tool surface; Part 11 covers implementation responsibilities per tool group, deployment models A and B, and the 34th tool `cello_request_human_input`
- [[2026-04-18_1454_merchant-crm-data-stash-and-free-samples|Merchant CRM Data Stash and Free Sample Tracking]] — four new MCP tools required: `cello_stash_write`, `cello_stash_read`, `cello_stash_read_all`, `cello_stash_delete`
- [[2026-04-19_2045_group-room-design|Group Room Design]] — adds six new MCP tools (`cello_invite_to_room`, `cello_petition_room`, `cello_get_room_info`, `cello_dissolve_room`, `cello_transfer_ownership`, `cello_request_floor`) and extends `cello_join_room` and `cello_create_room` with new parameters
- [[2026-04-24_1530_inference-billing-protocol|Inference Billing Protocol]] — proposes one new MCP tool (`tokenize_inference_content`) for the deferred hosted-tokenizer verification mode; inference rate card extends session policy carried by `cello_initiate_connection` and `cello_accept_connection`
