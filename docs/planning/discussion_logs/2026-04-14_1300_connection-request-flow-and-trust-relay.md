...
- [[2026-07-04_1730_cross-node-session-topology|Cross-node session topology]] — how this flow runs when initiator and target are homed on different directory nodes: the client moves to the target's node, keeping the negotiation (including human escalation) local to one directory; the attestation approach there preserves this log's mandatory-signal guarantee.
