...
- [[2026-04-17_1100_not-me-session-termination|"Not Me" Session Termination — Dual-Path Forced Abort]] — identifies the gap this log leaves open: K_server revocation blocks new FROST sessions but cannot close existing P2P sessions; designs the mechanism that fills the gap
- [[2026-04-17_1400_directory-relay-architecture-reassessment|Directory/Relay Architecture Reassessment]] — the session-level FROST decision made the directory/relay split possible: FROST at boundaries only means the directory can step away during the session while relay handles hash relay, sequencing, and Merkle tree building
- [[2026-04-24_1530_inference-billing-protocol|Inference Billing Protocol]] — inference rate card is bound to the session at the FROST establishment bookend and the rate card hash is embedded in every inference response leaf
