...
- [[2026-04-18_1454_merchant-crm-data-stash-and-free-samples|Merchant CRM Data Stash and Free Sample Tracking]] — key rotation must produce an identity continuity record so merchants can automatically migrate their per-contact data stash from old to new identity_key
- [[2026-06-25_2109_agent-identity-lifecycle-discovery|Agent Identity, Lifecycle & Discovery Model]] — frames this rotation as **account-authorized** re-key under a **stable `agent_id`** with an appended key-epoch chain (the "identity continuity record"), and binds accumulated reputation to the `agent_id` so rotation never resets it
