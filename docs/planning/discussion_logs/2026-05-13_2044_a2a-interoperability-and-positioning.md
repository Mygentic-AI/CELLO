...
The CELLO connection policy already handles "what will I accept." It has two orthogonal axes: the trust threshold (minimum verified signals required to connect) and the interaction scope (what the agent does and does not engage with, in the operator's own words). A pizza ordering bot declares its scope explicitly — it takes orders, confirms deliveries, handles order status. A highly-verified academic agent contacting it about theology is still violating the scope, regardless of their trust level. An agent that violates the declared scope is not simply filtered; they forfeit their bond. The Merkle-logged stated intent at connection time is the evidence. This is a meaningfully stronger posture than A2A's model, where a caller that violates expectations has no accountability trail. See [[2026-04-18_1357_connection-bond-usage-and-policy|Connection Bond Usage and Policy]].
