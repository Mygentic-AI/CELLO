...
- `client.startup.lock.write.failed` — level: warn; context fields: `{ reason }`; correlationId: false; fired by `acquireLockFile()` when writing the lock file fails (e.g., permission denied, disk full); `reason` is the error message; the process continues but orphan detection will be broken (non-fatal); added in CELLO-M6B-001; package: adapter-claude-code
