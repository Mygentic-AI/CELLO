...
- `client.startup.lock.released` — level: info; context fields: `{ pid }`; correlationId: false; fired by `releaseLockFile()` during signal handler execution (SIGTERM/SIGINT) when the lock file is successfully removed; `pid` is the current process's PID; signals clean shutdown and lock cleanup; added in CELLO-M6B-001; package: adapter-claude-code
