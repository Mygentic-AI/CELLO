...
- `frost.ceremony.failed` — level: warn; context fields: `{ agentId, reason, ceremonyId }`; correlationId: false; fired when `participateInCeremony` returns `ok: false` during session request handling (any reason); `reason` is one of `ceremony_timeout` (per-round timeout exceeded), `ceremony_exhausted` (max retry attempts exhausted), or `directory_below_threshold` (not enough directory nodes available to meet the FROST threshold); `agentId` and `ceremonyId` are truncated to 16 hex characters; no key material (share bytes, partial signatures) is included in the logged context (SI-001); added in M6B-002; package: directory
