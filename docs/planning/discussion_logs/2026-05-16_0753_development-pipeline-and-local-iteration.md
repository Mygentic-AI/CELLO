...
- `mmr.checkpoint.pending` — level: info; context fields: `{ sessionId, sealedRoot, stagedAt, correlationId }`; fired at `cello_close_session` when the FROST seal is complete but the MMR checkpoint has not yet committed the session's sealed_root; carries the seal-ceremony correlationId; added in PERSIST-017
