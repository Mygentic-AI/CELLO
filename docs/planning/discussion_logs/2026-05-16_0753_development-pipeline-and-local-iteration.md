...
- `client.startup.prior.process.killed` — level: info (SIGTERM) or warn (SIGKILL); context fields: `{ priorPid, signal }`; correlationId: false; fired by `acquireLockFile()` (cello-mcp startup) after successfully killing a prior process found in the lock file; `priorPid` is the process ID that was killed; `signal` is `"SIGTERM"` (graceful) or `"SIGKILL"` (escalation after 5s timeout); SIGKILL case is logged at warn level to distinguish force-kill from clean exit; ensures orphan process cleanup is logged; added in CELLO-M6B-001; package: adapter-claude-code
