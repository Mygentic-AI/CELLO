...
- `session.node.created` — level: info; context fields: `{ sessionId, agentName, peerId, multiaddr, correlationId }`; fired by `SessionNodeManager.createSessionNode()` after a new ephemeral libp2p node starts and its multiaddr is resolved; correlationId is minted per `createSessionNode` call; added in DAEMON-002; package: daemon
- `session.node.destroyed` — level: info; context fields: `{ sessionId, agentName, correlationId }`; fired by `SessionNodeManager.destroySessionNode()` after the ephemeral node has been stopped and its DB row marked sealed or interrupted; correlationId is threaded from the originating `createSessionNode` call (false — destroyed in a different call context, no correlation); added in DAEMON-002; package: daemon
...
- `session.node.create.failed` — level: error; context fields: `{ agentName, reason, correlationId }`; fired by `SessionNodeManager.createSessionNode()` when the libp2p node fails to start; the partially-created node is torn down before this event fires; correlationId is the one minted for the failed attempt; added in DAEMON-002; package: daemon
...
- `session.node.stop.failed` — level: error; context fields: `{ sessionId, error }`; fired by `SessionNodeManager.gracefulShutdown()` when an individual ephemeral node's `stop()` call rejects; shutdown continues to attempt remaining nodes; added in DAEMON-002; package: daemon
- `daemon.shutdown.failed` — level: error; context fields: `{ reason }`; fired by the daemon binary shutdown handler when `gracefulShutdown()` rejects; process exits 1 after logging; added in DAEMON-002; package: daemon
