...
- `session.node.created` — level: info; context fields: `{ sessionId, agentName, sessionPeerId, correlationId }`; fired by `SessionNodeManager.createSessionNode()` and `#createStandingReceiver()` after a new ephemeral libp2p node starts; correlationId is minted per creation call; added in DAEMON-002; package: daemon
- `session.node.destroyed` — level: info; context fields: `{ sessionId, agentName, reason }`; fired by `SessionNodeManager.destroySessionNode()` and `gracefulShutdown()` after the ephemeral node has been stopped and its DB row marked sealed or interrupted; `reason` is `sealed` | `interrupted` | `error`; correlationId: false (destroyed in a different call context); added in DAEMON-002; package: daemon
- `session.node.cap.reached` — level: warn; context fields: `{ agentName, currentCount, maxCount }`; fired by `SessionNodeManager.createSessionNode()` or `acceptSession()` when the 32-session cap is reached and the request is rejected; correlationId: false (rejection event, no session created); added in DAEMON-002; package: daemon
- `session.node.create.failed` — level: error; context fields: `{ sessionId, agentName, error, correlationId }`; fired by `SessionNodeManager.createSessionNode()` and `#createStandingReceiver()` when the libp2p node fails to start; the partially-created node is torn down before this event fires; correlationId is the one minted for the failed attempt; added in DAEMON-002; package: daemon
- `session.node.connection.rejected` — level: warn; context fields: `{ sessionId, attemptedPeerId, expectedPeerId }`; fired by `SessionConnectionGater` (for session nodes) and `DirectoryConnectionGater` (for the directory-facing node) when a connection attempt from an unexpected peer is denied; `sessionId` is the session's UUID for session nodes, or the sentinel `"__directory_facing__"` for the directory-facing node; correlationId: false (fires synchronously in the gater before any session context is attached); added in DAEMON-002; package: daemon
- `session.interrupt.db.write.failed` — level: error; context fields: `{ sessionId, error }`; fired by `SessionNodeManager.gracefulShutdown()` when the SQLite UPDATE to mark active rows as `interrupted` fails; the session node may still be stopped even if this fires; added in DAEMON-002; package: daemon
- `session.node.stop.failed` — level: error; context fields: `{ sessionId, agentName, error, correlationId }`; fired by `SessionNodeManager.gracefulShutdown()` and `destroySessionNode()` when an individual ephemeral node's `stop()` call rejects; shutdown continues to attempt remaining nodes; added in DAEMON-002; package: daemon
- `session.standing_receiver.permanently_unavailable` — level: error; context fields: `{ correlationId }`; fired by `SessionNodeManager.#createStandingReceiverWithRetry()` after 3 consecutive failed creation attempts; the daemon enters degraded mode where inbound sessions are blocked until restart; added in DAEMON-002; package: daemon
- `daemon.shutdown.failed` — level: error; context fields: `{ signal, error }`; fired by the daemon binary SIGTERM/SIGINT handler and the IPC logout handler when `gracefulShutdown()` rejects; `signal` is `"SIGTERM"`, `"SIGINT"`, or `"logout"`; binary exits 1 after logging; added in DAEMON-002; package: daemon
...
- `seal.certificate.legibility.built` — level: info; context fields: `{ sessionId, participantCount, finalMessageAnswered, correlationId }`; correlationId: the seal-ceremony correlationId (sessionIdHex); fired by `CelloDirectoryNode` in `processSeal` once per seal certificate after the receipt-not-assent legibility object is derived from the verified leaves; never logs message content, the disclaimer text, or anything readable as assent (frontier integers are safe); added in SESSION-004 (M7); package: directory
- `seal.certificate.delivery.encode_failed` — level: error; context fields: `{ sessionId, pubkeyHex, eventType, reason }`; fired by `#deliverOrEnqueue` / `#deliverFrostSealed` when encoding a `session_sealed` / `session_frost_sealed` frame throws (e.g. a malformed `legibility` field) — a derivation bug that re-fails on every retry, so the frame is dropped (NOT enqueued) and logged loudly; distinct from the send-failure path; `reason` uses `error instanceof Error ? error.message : String(error)`; added in SESSION-004 (M7); package: directory
- `seal.certificate.delivery.stream_dead` — level: warn; context fields: `{ sessionId, pubkeyHex, eventType, reason }`; fired by the seal-delivery methods when `#sendFrame` throws (the recipient's stream is dead) — distinct from an encode failure; the frame is then enqueued for deferred delivery; added in SESSION-004 (M7); package: directory
- `seal.certificate.persist.failed` — level: warn; context fields: `{ sessionId, reason }`; fired by the daemon's `registerSessionSealedListener` when persisting the normalised legibility certificate to client-side SQLite throws; the seal still completes (the cert flows through the live return path); added in SESSION-004 (M7); package: daemon (cello-client)
- `seal.certificate.frontier.unverifiable` — level: error; context fields: `{ sessionId, party, publishedFrontier, derivedFrontier, correlationId }`; fired by the client when a published `content_frontier` exceeds the maximum `last_seen_seq` it can re-derive from the party's signed local leaves (SI-002 client guard); RESERVED — the client re-derive guard is the DOD-LEG-2 follow-on increment; added in SESSION-004 (M7); package: daemon (cello-client)
- `session.seal.autoacknowledged` — level: info; context fields: `{ sessionId, responderPubkey, correlationId }`; correlationId: the session-id hex; fired by `SessionNodeManager.#maybeAutoAcknowledgeSeal` when B's node auto-co-signs + submits its responder SEAL leaf (via `submitSealLeaf`, signed with B's own K_local) on ingesting the counterparty's SEAL ctrl leaf — no agent close call; added in UPGRADE-002 (M7); package: daemon (cello-client)
- `session.seal.autoack.skipped` — level: warn (EXCEPT `content_tamper`, which is ERROR); context fields: `{ sessionId, reason, correlationId }`; fired by the auto-acknowledge verifiability gate when it does NOT auto-co-sign. `reason === "content_tamper"` (a content_hash mismatch was recorded for the session — genuine tamper, a SECURITY event) is emitted at ERROR and ALSO surfaces `counterparty_closing` to B's agent as a decision point (AC-002); a relay submit-failure reason is WARN and lets the agent close / reconnect complete the seal (never a silent half-seal). The `desynced` / `content_unverifiable` reasons are reserved for the MSG-001-3b canonical-sequence reconciliation follow-on. Added in UPGRADE-002 (M7); package: daemon (cello-client)
...
- `relay.manifest.updated` — level: info; context fields: `{ relayId, region, manifestVersion, healthCheckUrl }`; correlationId: false; fired by `RelayPoolManager.reSignManifestForRelay()` when the directory re-signs and uploads the relay manifest after a relay_register with a new healthCheckUrl; added in CELLO-M6B-006; package: directory
- `relay.manifest.update.failed` — level: error; context fields: `{ relayId, region, reason }`; correlationId: false; fired by the directory node relay_register handler when `reSignManifestForRelay()` throws (manifest re-sign or S3 upload failed); registration still succeeds — the relay_register_ok was already sent; added in CELLO-M6B-006; package: directory
- `relay.manifest.relay_missing` — level: warn; context fields: `{ relayId, region, reason }`; correlationId: false; fired by the directory node relay_register handler when a relay attempts to update the manifest but is not present in the manifest relays array — indicates a synchronization bug or the relay was removed from the manifest while its ECS task is still running; distinct from operational S3 failures; operations team needs to investigate manifest/registration state mismatch; added in CELLO-M6B-006; package: directory
...
- `registration.started` — level: info; context fields: `{ registrationId, channel, correlationId }`; correlationId: present; fired by `RegistrationStateMachine.handleNewUser()` immediately after the initial DB insert succeeds; `registrationId` is the new record UUID; `channel` is the channel type (e.g. `telegram`); added in OPS-AGENT-003; package: operations-agent
- `registration.phone.verified` — level: info; context fields: `{ registrationId, channel, correlationId }`; correlationId: present; fired by `RegistrationStateMachine.#handleAwaitingContact()` after the phone contact is verified and the record transitions to PHONE_CONFIRMED; raw phone number is never logged (SI-002); added in OPS-AGENT-003; package: operations-agent
- `registration.email.verified` — level: info; context fields: `{ registrationId, emailDomain, correlationId }`; correlationId: present; fired by `RegistrationStateMachine.#handleAwaitingEmailOtp()` after the OTP is verified and the record transitions to EMAIL_CONFIRMED; full email address is never logged — only the domain (SI-002); added in OPS-AGENT-003; package: operations-agent
- `registration.completed` — level: info; context fields: `{ registrationId, tokenId, correlationId }`; correlationId: present; fired by `RegistrationStateMachine` after the record transitions to PRE_AUTH_TOKEN_ISSUED and the token is delivered to the user; `tokenId` is the first 8 characters of the token (unique enough for log tracing, not the full token); added in OPS-AGENT-003; package: operations-agent
- `registration.expired` — level: info; context fields: `{ registrationId }`; correlationId: false; fired by `RegistrationEngine` when an active record is found to have passed its `expiresAt` — either during the periodic expiry sweep (`#runExpirySweep`) or on the next inbound message from the user; `registrationId` is the UUID of the expired record; added in OPS-AGENT-003; package: operations-agent
- `registration.otp.expired` — level: info; context fields: `{ registrationId, correlationId }`; correlationId: present; fired by `RegistrationStateMachine.#handleAwaitingEmailOtp()` when the OTP is expired at the time of submission; the record remains in AWAITING_EMAIL_OTP state and the user is asked to request a new code; added in OPS-AGENT-003; package: operations-agent
- `registration.otp.rate_limited` — level: warn; context fields: `{ registrationId, emailDomain, sendCount, correlationId }`; correlationId: present; fired by `RegistrationStateMachine.#isRateLimited()` when the OTP send rate limit for a given email domain has been exceeded within the current rate window; `sendCount` is the current count at the time the limit was reached; added in OPS-AGENT-004; package: operations-agent
- `registration.state.recovered` — level: info; context fields: `{ registrationId, state }`; correlationId: false; fired by `RegistrationEngine.start()` for each active registration record loaded from Postgres during startup; `state` is the current state of the recovered record; ensures in-memory `#activeRecords` map is populated for the contact-prompt sweep; added in OPS-AGENT-005A; package: operations-agent
- `registration.engine.error` — level: error; context fields: `{ "error.message", "error.stack" }`; correlationId: false; fired by `RegistrationEngine` when an uncaught error occurs in a periodic sweep timer callback or in `#handleInboundMessage`; `error.message` and `error.stack` are the error's string properties; added in OPS-AGENT-005A; package: operations-agent
...
- `client.startup.prior.process.killed` — level: info (SIGTERM) or warn (SIGKILL); context fields: `{ priorPid, signal, reaped? }`; correlationId: false; fired by `acquireLockFile()` (cello-mcp startup) after successfully killing a prior process found in the lock file; `priorPid` is the process ID that was killed; `signal` is `"SIGTERM"` (graceful) or `"SIGKILL"` (escalation after 5s timeout); SIGKILL case is logged at warn level to distinguish force-kill from clean exit; `reaped` (boolean, SIGKILL path only) is true when the kernel confirmed the process exited within 500ms of SIGKILL, false when the 500ms poll timed out (process is dying but not yet cleaned up — proceed anyway); ensures orphan process cleanup is logged without contradictory dual-event sequences; added in CELLO-M6B-001; package: adapter-claude-code
- `client.startup.lock.released` — level: info; context fields: `{ pid }`; correlationId: false; fired by `releaseLockFile()` during signal handler execution (SIGTERM/SIGINT/normal exit) when the lock file is successfully removed; `pid` is the current process's PID; signals clean shutdown and lock cleanup; added in CELLO-M6B-001; package: adapter-claude-code
- `client.startup.lock.read.failed` — level: warn; context fields: `{ reason }`; correlationId: false; fired by `acquireLockFile()` when reading the lock file fails with an error other than ENOENT (e.g., permission denied, I/O error); `reason` is the error message; the process continues and writes its own lock file (non-fatal); added in CELLO-M6B-001; package: adapter-claude-code
- `client.startup.lock.eperm` — level: warn; context fields: `{ priorPid, reason }`; correlationId: false; fired by `acquireLockFile()` when checking the prior process with `kill(priorPid, 0)` returns EPERM (process owned by another user); `priorPid` is the PID from the lock file; `reason` is "process owned by another user"; the lock is treated as stale and the current process continues; added in CELLO-M6B-001; package: adapter-claude-code
- `client.startup.lock.check.failed` — level: warn; context fields: `{ priorPid, reason }`; correlationId: false; fired by `acquireLockFile()` when checking the prior process with `kill(priorPid, 0)` fails with an unknown error (not ESRCH or EPERM); `priorPid` is the PID from the lock file; `reason` is the error message; the lock is treated as not running and the current process continues; added in CELLO-M6B-001; package: adapter-claude-code
- `client.startup.lock.write.failed` — level: warn; context fields: `{ reason }`; correlationId: false; fired by `acquireLockFile()` when writing the lock file fails (e.g., permission denied, disk full); `reason` is the error message; the process continues but orphan detection will be broken — a no-op cleanup function is returned and the operator should diagnose the filesystem permission issue; added in CELLO-M6B-001; package: adapter-claude-code
- `client.startup.prior.kill.failed` — level: warn; context fields: `{ priorPid, signal, reason }`; correlationId: false; fired by `acquireLockFile()` when sending a signal (SIGTERM or SIGKILL) to the prior process fails; `priorPid` is the target PID; `signal` is the signal that was sent; `reason` is the error message; typically happens when the process exited between the check and the kill (race condition); the current process continues (non-fatal); added in CELLO-M6B-001; package: adapter-claude-code
- `client.startup.lock.acquired` — level: info; context fields: `{ pid }`; correlationId: false; fired by `acquireLockFile()` after successfully writing the PID to the lock file; `pid` is the current process's PID; signals that lock acquisition succeeded and the process is now the sole cello-mcp instance for this agent; added in CELLO-M6B-001; package: adapter-claude-code
- `client.startup.lock.release.failed` — level: warn; context fields: `{ reason, pid }`; correlationId: false; fired by `releaseLockFile()` when `unlinkSync(lockFilePath)` fails with an error other than ENOENT (e.g., EACCES, EROFS); `reason` is the error message; `pid` is the current process's PID; non-fatal — the process is already exiting; operator should diagnose the filesystem permission issue; added in CELLO-M6B-001; package: adapter-claude-code
- `frost.ceremony.failed` — level: warn; context fields: `{ agentId, reason, ceremonyId }`; correlationId: false; fired when `participateInCeremony` returns `ok: false` during session request handling (any reason); `reason` is one of `ceremony_timeout` (30-second wait for ceremony_result from client timed out), `ceremony_exhausted` (client-side FROST ceremony returned null signature — agent not bootstrapped), or `directory_below_threshold` (ClientDelegatedSigner delegation broken: #streams is null, agent not in #streams map, or stream.send threw); `agentId` and `ceremonyId` are truncated to 16 hex characters; no key material (share bytes, partial signatures) is included in the logged context (SI-001); added in M6B-002; package: directory
- `registration.already_registered.warned` — level: info; context fields: `{ registrationId, channelUserId, existingRegistrationAge }`; correlationId: false; fired by `RegistrationEngine.#handleInboundMessage()` when a user messages the bot but already has a `PRE_AUTH_TOKEN_ISSUED` registration created within the last 30 days; `registrationId` is the UUID of the existing completed registration; `channelUserId` is the channel user identifier; `existingRegistrationAge` is the age of the existing registration in milliseconds; the bot sends a re-registration warning and waits for `CONFIRM` before creating a new registration; added in CELLO-M6B-011; package: operations-agent

**Directory Signaling (M7-DIR-PING-001)**

- `directory.signaling.ping.received` — level: debug; context fields: `{ clientPubkey, streamId }`; correlationId: false; fired by the signaling stream frame dispatch when an authenticated client sends a `{ type: "ping", ts }` heartbeat frame; `clientPubkey` is the full hex pubkey of the authenticated client; `streamId` is the libp2p stream identifier; added in M7-DIR-PING-001; package: directory
- `directory.signaling.pong.sent` — level: debug; context fields: `{ clientPubkey, streamId }`; correlationId: false; fired immediately after the directory successfully sends a `{ type: "pong", ts }` response echoing the client's monotonic counter; added in M7-DIR-PING-001; package: directory
- `directory.signaling.pong.failed` — level: debug; context fields: `{ clientPubkey, streamId, error }`; correlationId: false; fired when `#sendFrame` throws during pong delivery (stream closed, write error); `error` is the error message string (never the raw error object); the stream is NOT terminated — subsequent frames on the same stream are still processed; added in M7-DIR-PING-001; package: directory
