...
- `relay.manifest.poll.started` — level: info; context fields: `{ intervalMs }`; correlationId: false; fired by `RelayPoolManager.startPolling()` immediately after starting the `setInterval` loop; `intervalMs` is the configured poll interval (default: 120000 ms from `RELAY_MANIFEST_POLL_INTERVAL_MS`); fired only in CELLO_ENV != 'local' (local dev has no S3); added in CELLO-M6B-008; package: directory
- `relay.manifest.refreshed` — level: info; context fields: `{ manifestVersion, relayCount }`; correlationId: false; fired by the manifest poll loop inside `RelayPoolManager.startPolling()` when a poll retrieves a manifest with `version > currentVersion` and `applyManifest()` updates the in-memory relay pool; `manifestVersion` is the new version number; `relayCount` is the number of active relays in the updated pool; the authoritative "poll picked up new manifest" signal; `relay.manifest.loaded` fires only on directory startup (not during polling) — the poll loop uses `suppressLoadedLog=true` to avoid dual events for the same occurrence; added in CELLO-M6B-008; package: directory
- `relay.manifest.poll.noop` — level: debug; context fields: `{ currentVersion, receivedVersion }`; correlationId: false; fired by the manifest poll loop inside `RelayPoolManager.startPolling()` when a poll retrieves a manifest whose version is not strictly higher than `currentVersion` (S3 not yet updated, or poll interval shorter than manifest propagation time); `currentVersion` is the version currently held in memory; `receivedVersion` mirrors `currentVersion` when the stale-version check fires from the catch path (approach A: snapshot-and-compare — `receivedVersion` reflects the pre-poll snapshot, not a parsed manifest field); no pool update is applied; added in CELLO-M6B-008; package: directory
- `relay.manifest.poll.failed` — level: warn; context fields: `{ reason, currentVersion }`; correlationId: false; fired by the manifest poll loop catch handler inside `RelayPoolManager.startPolling()` when `loadManifest()` throws with an error that is NOT a stale-version rejection (i.e., `reason` does not start with "Manifest rejected: version"); covers S3 unavailable, network errors, and signature verification failures (the last also fires `relay.manifest.invalid` at ERROR before throwing); `reason` is the error message; `currentVersion` is the in-memory version at the time of failure; the poll loop continues after logging — a single failure does not stop future polls; added in CELLO-M6B-008; package: directory
