...
- `client.startup.progress` — level: info; context fields: `{ step, outcome, durationMs }`; correlationId: false; fired conceptually for each named startup step in cello-mcp (database open, bootstrap fetch, directory dial, state load, ready); in practice emitted as human-readable lines to stderr in format `cello: <step>... <outcome>` — the structured event name is the canonical name for on-call reference; `step` is one of `opening_database`, `fetching_directory_address`, `connecting_to_directory`, `loading_agent_state`, `ready`; `outcome` is `ok` or `failed`; added in M6-DX-001; package: adapter-claude-code
