...
- `session.seal.autoack.skipped` — level: warn; context fields: `{ sessionId, reason, correlationId }`; fired by the auto-acknowledge verifiability gate when it does NOT auto-co-sign — `reason` is `content_unverifiable` (a content_hash mismatch = tamper was recorded for the session) or a relay submit failure reason; the seal may then complete via the agent close / reconnect (never a silent half-seal); added in UPGRADE-002 (M7); package: daemon (cello-client)
