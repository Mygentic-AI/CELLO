...
- `relay.session.idle.swept` — level: info; context fields: `{ sessionId, idleDurationMs }`; correlationId: false; fired by the relay's idle-sweep timer when a session with status=active and lastActivityAt older than maxIdleMs is destroyed; emitted by `sweepIdleSessions()` once per swept session; idleDurationMs = (Date.now() - lastActivityAt); added in CELLO-M6B-009; package: relay
- `relay.session.sweep.complete` — level: info; context fields: `{ sweptCount, remainingCount }`; correlationId: false; fired by the relay's idle-sweep timer at the end of each sweep pass; `sweptCount` is the number of sessions destroyed in this pass; `remainingCount` is the total number of active+sealed sessions in the store after the sweep; added in CELLO-M6B-009; package: relay
- `directory.pool.max.high` — level: warn; context fields: `{ configured, recommended_max }`; correlationId: false; fired by `resolvePoolMax()` at directory startup when DIRECTORY_PG_POOL_MAX is set to a value > 100; advisory only — the directory starts normally with the configured value; added in CELLO-M6B-009; package: directory
...
- `relay.session.idle.swept` — level: info; context fields: `{ sessionId, idleDurationMs }`; correlationId: false; fired by `InMemoryRelayStore.sweepIdleSessions()` when an idle session is destroyed by the sweep; `sessionId` is the hex string of the session identifier; `idleDurationMs` is how long the session was idle before being swept; added in CELLO-M6B-009; package: relay
- `relay.session.sweep.complete` — level: info; context fields: `{ sweptCount, remainingCount }`; correlationId: false; fired by `InMemoryRelayStore.sweepIdleSessions()` after each sweep cycle completes; `sweptCount` is the number of sessions destroyed during this sweep; `remainingCount` is the number of sessions still in the store after the sweep; added in CELLO-M6B-009; package: relay
- `directory.pool.max.high` — level: warn; context fields: `{ configured, recommended_max }`; correlationId: false; fired by the directory composition root at startup when `DIRECTORY_PG_POOL_MAX` is set above 100 (the soft ceiling for db.t3.medium shared across 3 nodes); `configured` is the actual poolMax value from the env var; `recommended_max` is 100; the process starts successfully and uses the configured value — the warning is advisory; added in CELLO-M6B-009; package: directory
- `relay.delivery.queue.full` — level: warn; context fields: `{ pubkeyHex }`; correlationId: false; fired by `InMemoryRelayStore.enqueueDelivery()` when the delivery queue for a pubkey reaches the 256-frame bound and the oldest frame is dropped to make room for the new one; `pubkeyHex` is the first 16 hex characters of the recipient's public key (truncated for log size); added in CELLO-M6B-009; package: relay
