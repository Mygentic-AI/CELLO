...
- Voucher accountability status for recovery contacts: 90-day liability window from recovery date; lockout indicator if triggered (6-month lockout on first bad outcome); rolling 2-month cap indicator (1 attestation per 2-month window during the recovered agent's probationary period; rises to 3 per 2-month window after probation completes)
...
| ~~F-39~~ | Portal | ~~Resolved~~ — Succession package creation is a `portal_instruction` to the client; the client performs all encryption locally (identity key private bytes → successor's `identity_key`); the portal never handles key material. Portal UI shows package status and successor handle; WebAuthn required to trigger. Successor's `identity_key` resolved at designation time. |
