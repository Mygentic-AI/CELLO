...
| M3 — Connections & Policy | Registration (stubbed OTP), connection request flow, trust data relay with selective disclosure, `SignalRequirementPolicy` evaluation, accept/decline. Connection package items (pseudonym binding, attestations, endorsements) are signed with ML-DSA via `liboqs` / `node-oqs` — this is the first milestone where ML-DSA is load-bearing. The ML-DSA security level (44 vs 65) must be decided before M3 stories are written. | Agents control who reaches them. A receiver can require specific trust signals and enforce it. The one-round negotiation works — what you disclose, what you withhold. |
