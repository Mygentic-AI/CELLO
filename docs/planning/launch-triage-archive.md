---
name: Launch Triage Archive
type: triage
date: 2026-08-21
topics: [launch, archive, closed-items]
status: closed
description: >
  Closed/withdrawn items moved out of docs/planning/launch-triage.md on 2026-08-21 to keep that
  doc a working punch list rather than a history. Everything here is dispositioned — fixed,
  deployed and verified, or withdrawn as never having been a real defect. Kept for lookup so a
  defect already found and fixed isn't re-discovered as new. See launch-triage.md for what is
  still open.
---

# Launch Triage — Archive

Moved here from `launch-triage.md` on 2026-08-21 (Andre: "shorten this file"). Content is
unchanged from the main doc — this is a relocation, not a rewrite. Cross-references to
`[[launch-triage]]` and milestone DoD docs are preserved as they were.

## Index

- Anyone can vouch for their own agent using a key they made thirty seconds ago — `DOD-END-ISSUER-REGISTERED-1` — fixed, reviewed, live 2026-08-10
- One email address maps to two different accounts, depending on which node you ask — `DOD-EMAIL-STUB-FORK-1` — fixed same day found, 2026-08-10 (junk row, not a code defect)
- The inbox says sessions are sealed when they are not — `DOD-SEALED-INBOX-2` — fixed and live, daemon 0.0.134
- Dead signaling streams go undetected — `DOD-SIGNALING-LIVENESS-1` — root cause fixed 2026-08-06
- The Telegram sign-up messages give wrong or unclear instructions — `D-ENVVAR` — fixed 2026-08-09, proven live
- Installing the plugin strands a new user at `daemon_not_running` — fixed 2026-08-09, connect 0.0.137
- "Online" does not mean reachable — fixed and promoted 2026-08-09, daemon 0.0.155 / cli 0.0.162 / connect 0.0.139
- The relay stops being able to notarize, never recovers, and reports itself perfectly healthy — `DOD-RELAY-DIRECTORY-RECONNECT-1` — deployed and verified 2026-08-08
- Trust-signal replication fails every round, and the fork alarm climbs — `DOD-SIGNAL-REPLICATION-1` — fixed, deployed and verified 2026-08-09
- A superseded trust signal still reads as current on two nodes out of three — `DOD-SIGNAL-STATUS-REPLICATION-1` — withdrawn as filed, never a real defect
- A write deleted a peer's edit it never saw — `DOD-DOC-STALE-WRITE-1` — shipped 2026-08-09, proven live across two machines
- Nested fields in a shared JSON document did not merge — `DOD-DOC-JSON-NESTED-1` — shipped 2026-08-09, proven live
- Document types: JSON and HTML were missing, plaintext's diff was dead — `DOD-DOC-TYPES-1` — shipped 2026-08-09
...
- Filed 2026-08-09 from the `DOD-SIGNAL-REPLICATION-1` unit review — three guard gaps, none armed, none live
