...
- **Key rotation flow**: Owner authenticates with WebAuthn at the portal → portal issues a short-lived, single-use rotation authorization token (signed by portal keypair, scoped to `agent_id` + `rotate_k_local`) → portal delivers token to the agent client via `portal_instruction` (action: `initiate_rotation`) → client generates new K_local_v2 locally (key material never transits the portal) → client submits `{new_pubkey, rotation_token}` directly to the directory → directory validates the portal signature on the token, retires K_local_v1, runs a new K_server_X ceremony paired with K_local_v2 → new public keys published; old public keys marked expired with timestamp; all agents that cached old keys are notified to refresh. The portal never touches K_local material — it only authorizes the operation.
...
**[~~GAP G-44~~ — Resolved]**: All succession gaps are closed. Server-side mechanics, client-side state machine (AC-81, AC-82), and all portal screens (F-32, F-33, F-34, F-39) are fully specified. Succession is implementable end-to-end.
...
- Rate limits: max **100 new endorsements per rolling 30 days** per owner (phone number) — shared across all agents under that owner to prevent farming via multiple agents. Cap and window are **configuration, not literals**; raising them is a config change, no code edit and no migration. **[GAP G-17 RESOLVED — superseded 2026-07-28 by M10B-D6]**
  > **Amended 2026-07-28 (M10B-D6).** Was *10 per month*. Three changes: the number is 100; the window is a **rolling 30 days** (a calendar month gives every farmer a burst at reset); and enforcement moves to the **portal**, because that is where minting happens under the §6 amendment — the directory notarizes a hash and cannot count what it cannot read. The *per-owner* scope is unchanged and was always right: a per-agent cap is bypassed by spinning up agents, which is the hole the same-owner rule exists to close. Over-quota is refused with a named cause, the cap, and when the window frees up. Implementation: `DOD-END-QUOTA-1` in [[M10B-DEFINITION-OF-DONE]].
...
- **Notification type registry**: Enumerated, not freeform. Full type list:
  ...
  | `ATTESTATION_RECEIVED` | Directory | Another agent issued an attestation for this agent |
  ...
  | `KEY_ROTATION_RECOMMENDED` | Directory | Directory is nudging the agent to rotate K_local (pre-rotation scheduling nudge); payload: `{ agent_id, recommended_at }` — no epoch data, rotation has not yet occurred |
...
**Threat model note**: "Not Me" is an availability brake, not a confidentiality mechanism. A SIM-swap attacker who sends "Not Me" from a hijacked phone can burn K_server_X and lock the agent — but cannot re-key, because re-keying requires WebAuthn or TOTP (which the attacker does not have). The result is a DoS: the legitimate owner's agent is bricked until they recover via social recovery or re-enroll a second factor. Owners who have not enrolled WebAuthn or TOTP must recover via M-of-N social recovery, which carries its own 48-hour contest window. This is the designed tradeoff: the emergency brake is available to anyone who controls the registered phone number, and the re-keying gate prevents that access from becoming a takeover.
...
| G-8 | Directory/FROST | ~~Resolved~~ — Epoch ID: `agent_id:epoch:N` (monotonic integer). Grace period: 7 days. Hard cutoff after grace. Post-rotation `KEY_ROTATION_COMPLETED` payload: `{ agent_id, old_epoch, new_epoch, old_pubkey, new_pubkey, rotation_timestamp, expires_at }`. Pre-rotation nudge `KEY_ROTATION_RECOMMENDED` carries no epoch data. |
...
| G-17 | Directory → Portal | ~~Endorsement rate limit N not specified~~ **RESOLVED**, amended 2026-07-28 (M10B-D6): **100 new endorsements per rolling 30 days** per owner (phone number), shared across all agents under that owner, cap+window configurable, enforced at the **portal** (where minting happens). Was 10/month at the directory. |
...
- [[agent-client|CELLO Agent Client Requirements]] — the client-side counterpart to this document; the client and directory together implement the full protocol; shared conflicts (C-1, C-3, C-4, C-5, C-6, C-7 all resolved) are cross-referenced throughout both documents
...
- [[2026-04-17_1000_trust-signal-pickup-queue|Trust Signal Pickup Queue]] — G-5 resolved here: the encrypted async pickup queue for oracle-verified JSON blob delivery; portal encrypts to identity_key, directory stores ciphertext with 30-day TTL, TRUST_SIGNAL_PICKUP_PENDING notification type
