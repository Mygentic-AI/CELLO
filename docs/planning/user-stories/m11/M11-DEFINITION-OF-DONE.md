...
- **DOD-GALLERY-1** [corp-cello-site] — gallery live at `gallery.cello.mygentic.ai` in corp-cello-site repo. Corp site header + footer, same design system. Corp site nav gains a "Gallery" item linking to the gallery index. No auth required on any gallery page. SSR-rendered (bot-indexable). Robots.txt allows all crawlers on `/gallery/*`. — 🟡 `/gallery` live in corp-cello-site with header, footer, nav item and no auth; robots.txt already allows every crawler on everything. 🟡 **DOWNGRADED FROM ✅ 2026-07-29 by `cello-done-auditor` — the ✅ was mine and it was wrong.** The line says **"SSR-rendered (bot-indexable)"** and that clause is met at **zero percent**, not partly: the served HTML for the index and for a receipt contains no moniker, no hash and no transcript (grep of the live response), `npm run build` marks every route `○ (Static)`, and `sitemap.xml` carries one `/gallery` entry and no per-receipt URL — so even a JS-executing crawler finds one page, not five. `robots.txt` explicitly invites GPTBot to a page that serves it an empty shell. Tagging this ✅ would silently retire the **parked three-way fork** (Journal Entry 35) that is still Andre's to decide. Everything else IS earned and verified: `gallery.cello.mygentic.ai` resolves and serves — A record in the `cello.mygentic.ai` zone, certificate expanded to carry the name in the same deploy (the block presents the `cello.mygentic.ai` cert, so nginx and the SAN list must move together), page 200, API namespace reachable from that host. Header, footer, nav item, no auth, robots unrestricted → Journal Entries 32, 33, 61, 62, 64
