...
- **DOD-LANDING-1** — landing page live at the primary `cello.mygentic.ai` URL (or a `waitlist.cello.mygentic.ai` staging equivalent) in corp-cello-site repo. Contains: headline, subhead, one-field email form, submit button. No tracking pixels, no third-party scripts. Design: corp site look/feel. — ❌
...
- **DOD-E1-1** — E1 (confirmation) email template deployed: subject, plain-text + HTML. Body: confirmation that they're on the list, their queue position (computed view), their personal referral link, a one-sentence "how waves work" note. Sent within 60 seconds of signup. Verified: signup with a verified address, check inbox, confirm queue position is a real number. — ❌
...
- **DOD-OPS-DASHBOARD-1** — ops dashboard live at `operations.cello.mygentic.ai`. New repo (cello-portal clone). Auth: magic link only. Allowed emails from AWS Secrets Manager key `cello/ops/allowed-emails` (JSON array). Magic link to an unknown email → silent rejection. Pages:
...
- **DOD-BLOG-INFRA-1** — Ghost blog at `blog.cello.mygentic.ai`. Google Search Console property verified. GA4 property created and tracking script deployed. Every article published with: FAQPage JSON-LD schema, Article schema with `datePublished` + `dateModified`, year in URL slug, visible "Last updated" line below H1. Verified: publish a test article; confirm Google Search Console shows it as crawlable; confirm GA4 shows a page view. — ❌
