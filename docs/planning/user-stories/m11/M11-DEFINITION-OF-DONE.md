...
- **DOD-GALLERY-1** [corp-cello-site] — gallery live at `gallery.cello.mygentic.ai` in corp-cello-site repo. Corp site header + footer, same design system. Corp site nav gains a "Gallery" item linking to the gallery index. No auth required on any gallery page. SSR-rendered (bot-indexable). Robots.txt allows all crawlers on `/gallery/*`. — ✅ `/gallery` live in corp-cello-site with header, footer, nav item and no auth; robots.txt already allows every crawler on everything. ✅ **EARNED 2026-07-29.** Reached ✅ once wrongly (the auditor caught it) and then honestly. The blocking clause was **"SSR-rendered (bot-indexable)"**, which was met at **zero percent**: the served HTML for the index and for a receipt contains no moniker, no hash and no transcript (grep of the live response), `npm run build` marks every route `○ (Static)`, and `sitemap.xml` carries one `/gallery` entry and no per-receipt URL — so even a JS-executing crawler finds one page, not five. `robots.txt` explicitly invites GPTBot to a page that serves it an empty shell. **Now delivered by pre-rendering (option (a) of the Entry 35 fork, chosen and shipped — see Parked).** Each receipt is a real HTML file carrying its conversation plus `Conversation`/`Message` JSON-LD; the index cards are in the file; the sitemap lists all five from the same build-time read that generates them. Verified by fetching as the crawlers themselves: `curl -A GPTBot` on a receipt returns the monikers, the turns and the JSON-LD, and `curl -A PerplexityBot` on the index returns the cards. The clause is satisfied by static generation rather than a server, which is what "bot-indexable" was asking for. Also earned and verified: `gallery.cello.mygentic.ai` resolves and serves — A record in the `cello.mygentic.ai` zone, certificate expanded to carry the name in the same deploy (the block presents the `cello.mygentic.ai` cert, so nginx and the SAN list must move together), page 200, API namespace reachable from that host. Header, footer, nav item, no auth, robots unrestricted → Journal Entries 32, 33, 61, 62, 64
...
- **DOD-GALLERY-INDEX-1** [corp-cello-site] — gallery index at `gallery.cello.mygentic.ai/` shows a card grid of published receipts: agent monikers, timestamp, verification badge. Cards link to the receipt page. Supports at least basic pagination (20 per page). — ✅ card grid + pagination over the real total; an empty gallery says so rather than showing sample cards. **The live run found the page broken and it is now fixed**: the client prefixed the waitlist base to a gallery path, so every call 404'd and the page rendered API Gateway's `"Not Found"` — the empty-state branch had never executed in production. Proven live on `cello.mygentic.ai` (`200 {"receipts": [], "total": 0}`). **✅ EARNED 2026-07-29:** the index serves five real receipts on the host this line names, cards carrying monikers, day-precision date, message count and the seal state each session actually had → Journal Entries 32, 33, 61, 62, 63, 64

- **DOD-GALLERY-CONTENT-1** [corp-cello-site] — a published receipt carries the session transcript, and the receipt page renders it. Per M11-D33: `published_receipts` gains a transcript (ordered turns: speaker moniker + body, sequence preserved) and the receipt page shows the exchange below the seal metadata. Content is subject to the same irrevocability as the hash — there is no edit and no delete path. Turn bodies are caller-supplied strings on a public, bot-indexed page, so they are refused at the write on the same terms as monikers, never escaped at the read. A receipt with no transcript renders exactly as it does today rather than showing an empty panel. **Open and deliberately not solved here:** a transcript contains the counterparty's words and one party publishes it — acceptable for the seeded archive because both agents are Andre's, and a blocker for third-party publishing that `DOD-GALLERY-PRIVACY-1`'s portal action must answer before it ships. — ✅ **EARNED 2026-07-29:** transcripts stored, served and rendered in order; markup refused at the write by a validator now shared with the seeder (it previously ran only on the API path, which had written nothing); `message_count = jsonb_array_length(transcript)` enforced by constraint `0025`; a receipt without a transcript renders no panel, revert-tested → Journal Entries 63, 64

- **DOD-GALLERY-SEED-1** [corp-cello-site] — the build-in-public sessions in `docs/planning/milestone-writeups/live-session-e2e-proofs/` (trustless-cello) that carry a sealed root are published to the gallery, each rendering the seal state it actually had. **Five of the 14 files qualify** (m4 ×3, m8b, m8c) — measured by extraction, not estimated. Six have no sealed root because those sessions were never sealed (m0 ×3, m2 ×2, m3 ×2), and `receipt_hash` is the primary key and the only thing a visitor can check, so a session with no root cannot be a receipt without inventing the hash the page exists to verify. `smoke-test-m4` has a root but no transcript and covers TWO sessions, so the root cannot be attributed to one bilateral exchange; excluded. The collection write-up is not a session. **Cross-check that makes `message_count` verified rather than asserted:** extracted turns reconcile with the leaf counts each document states independently — 25 turns/27 leaves, 10/12, 5/7 (genesis + turns + seal), and m8b states "7 content messages" for 7 extracted turns. Per M11-D34: `seal_status` is stored and displayed; `verified_by`/`node_count` are populated ONLY where the record states them and are otherwise null, with the page showing the real state (`sealed`, `sealed — attestations pending`, `seal_deferred`) instead of a badge. No count is inferred, defaulted, or back-filled from "we run three nodes". Verified: the live index lists the archive; a receipt page renders its transcript; and every published verification number traces to a line in the source document. — 🟡 **DOWNGRADED FROM ✅ 2026-07-29 by `cello-done-auditor`.** Earned and live-verified: five receipts in the portal DB, **zero** carrying an attestation claim, and an unrecognised seal state refused rather than coerced to `sealed`. **Two of the three verification claims do not hold as written.** (a) "each field asserted against its source document" describes a test that never opens the source document — it compares parser output to a hand-written `EXPECTED` dict in the same file, so it cannot catch a value transcribed wrongly, which is precisely the failure this line already suffered twice. (b) m8c's `message_count` of 4 has **no** corroborating leaf or message count anywhere in its document; the reconciliation covers four of five, not five. (c) "every published verification number traces to a line in the source document" is vacuous — no verification numbers are published → Journal Entries 63, 64, 65
...
## P4 — GCP port (M12 cutover item C, landing under M11)

**Why these lines are in M11 and not M12.** The waitlist is M11's deliverable and this document is
its sole status authority (§1 of [[M11-PROCEDURE]]). M12 owns the cloud; M11 owns the waitlist. The
work is tracked as item C in [[M12-CUTOVER-CHECKLIST]] for coordination — that file is where agents
avoid collisions, not where waitlist status lives. See M11-D35.

**Nothing in this tier needs AWS awake** (verified 2026-07-31): the handlers and the 26 migrations
are in the repos, the target Cloud SQL and Cloud Run are on GCP, and Route 53 / SES / Secrets
Manager are precisely what `hibernate.sh` leaves running. Deleting the AWS waitlist stack is NOT in
this tier — it is M12 P4 teardown, gated on Andre's explicit go.

**It is a port, not a rewrite.** The 8,468 lines of existing tests call the handlers directly and
are the yardstick. A test that needs EDITING to pass is the signal the port became a rewrite: stop
and surface it rather than pushing through.

- **DOD-GCP-SCHEMA-1** [corp-cello-site, trustless-cello] — the 26 waitlist migrations are applied to the **GCP** portal Cloud SQL (`cello-infra:us-east1:cello-portal`, database `cello_portal`), alongside the portal's existing 11, in the shared `schema_migrations` ledger. All **19** waitlist tables exist, plus the `waitlist_queue` view, and `queue_position` is NOT a stored column (`DOD-QUEUE-VIEW-1`). The ledger holds exactly 37 rows. Idempotent: a second run applies 0. No renumbering of either set — the ledger keys on the full stem (`0001_init` vs `0001_m11_waitlist_p0`), so both starting at `0001` is correct and must stay that way. `DOD-INV-SINGLE-DB` still holds: one database, additive schema, no new instance provisioned. — ✅ **EARNED 2026-07-31** by `infra/scripts/verify-gcp-waitlist-schema.sh`, which was written BEFORE the migrations ran and failed `clause 1 — 20 of 20 waitlist tables missing`. After applying: `clause 1 — all 19 waitlist tables + the 1 view present, position not stored` · `clause 2 — ledger holds 37 rows` · `clause 3 — second run applied 0`. Applied by the real `waitlist-migrate` handler (`MIGRATIONS_DIR` + `DATABASE_URL`, both env knobs it already exposes), not a reimplementation: 26 applied, `0001_m11_waitlist_p0` → `0026_receipt_title_and_summary`. **The enforcer's own first draft said 20 tables and named a table called `skips`** — a regex that read the prose "CREATE TABLE IF NOT EXISTS skips the table wholesale" out of a comment in `0002`. It failed loudly because the check requires each name to be present; written as "no unexpected tables" it would have passed silently. **Reviewed, and every blocking finding was in the ENFORCER rather than the migration work — see Entry 68.** The worst: the idempotency clause ran the migrator in APPLY mode, so a checkout carrying an unmerged migration would have written it to the live database and then blamed the migrator ("not idempotent") for what was checkout drift. Now a dry run, which keeps the checksum and stem-key guards and loses the write. Also fixed: a stale checkout passed all three clauses green; the ledger check asserted `count(*)=37` (an arithmetic identity 12+25 also satisfies) and now asserts every stem by name; `body.get("applied", [])` defaulted to the passing value; the password came from a `sed` that passes input through unchanged on a non-match. Tripwires verified to bite and to name the checkout, not the database → Journal Entries 67, 68

- **DOD-GCP-ROUTER-1** [trustless-cello] — an HTTP entry layer that lets the 13 handlers run under Cloud Run unchanged. The API Gateway v2 unwrap (`requestContext.http.method/path`, `event["body"]`, the top-level `cookies` list that broke `DOD-AUTH-1` once already) becomes a request adapter; the 4 HTTP-facing handlers (signup, auth, actions, gallery — 21 routes) are reached by path exactly as today. The 5 direct-invoke handlers (migrate, waves, gate, firstwin, utm) get an authenticated internal invoke path — a path router alone does not reach them. `_dispatch.py`'s Lambda invoke becomes an HTTP POST to the email service. **Handler logic is not modified.** Verified: the existing 8,468 lines of tests pass unedited. — ✅ **EARNED 2026-07-31.** `_router.py` (adapter + route table + fail-closed internal guard), `_app.py` (the WSGI application), `_dispatch.py` (Lambda invoke → HTTP POST on a daemon thread, so the caller never waits for a drain). **587 tests pass and no `handler.py` is in the diff.** Two test files changed and both are disclosed rather than glossed: `test_sqlstate.py` built its "refused port" case by string-replacing a hardcoded `:55432`, which silently no-ops under any other `PGURL` and left the case pointed at the live database; and the gate race test asserted a symptom (`len(allowed) == 1`) that failed on correct behaviour 9 runs in 100 — replaced with the harm its own docstring names, plus `live == 1`, the "no grant was lost" half nothing had ever asserted. **Reviewed; two blocking defects found that the tests structurally could not see, both fixed and revert-tested** — `waitlist-gallery` could not be imported at all (the loader never put the handler's own directory on `sys.path`, so the whole public gallery API 500'd and every email failed inside `render()`), and the internal payload wrapper discarded the top-level parameters of four targets, so `{"dry_run": true}` to migrate **applied** the migrations and a scheduled sweep ran a drain and reported success → Journal Entry 69

- **DOD-GCP-RUNTIME-1** [trustless-cello] — Cloud Run service + the **4** Cloud Scheduler jobs in Terraform, matching the CFN rules exactly: email drain `rate(1 minute)`, feedback sweep `cron(17 6 * * ? *)`, re-engage `cron(23 6 * * ? *)`, outreach sweep `cron(47 6 * * ? *)`. SES bounce/complaint SNS posts to a Cloud Run URL instead of invoking a Lambda. `DATABASE_URL` resolves to the GCP Cloud SQL; `_dburl.py` is unmodified because it already prefers that variable. SES keeps sending, with the static credentials the ops-agent already proves work from GCP. — ✅ **EARNED 2026-07-31, DEPLOYED AND SERVING.** `cello-waitlist` on Cloud Run us-east1 (image `waitlist-963fb277`, min=1), all 4 Cloud Scheduler jobs ENABLED and reporting OK. Verified against the deployed service, not the plan: `/health` 200 · `/gallery/receipts` 200 `{"receipts": [], "total": 0}` querying the real GCP Cloud SQL · `/internal/*` 401 without a token · the drain answers `{"sent": 0, "skipped": 0, "failed": 0, "retired": 0}` and the re-engagement sweep `{"re_engage_enqueued": 0}`. **Two defects were found only by running it**, both fixed and redeployed: four env vars the CFN template set and the Terraform did not (`WAITLIST_SES_CONFIG_SET`, whose absence made `waitlist-email` refuse outright, plus `WAITLIST_SITE`/`WAITLIST_API_BASE`/`WAITLIST_FROM_EMAIL`, which build the confirm link inside E1 and fail silently), and a response adapter that discarded every bare payload — the handlers without an HTTP trigger return their result directly, so the drain answered an empty 200 and a successful send was indistinguishable from a no-op, on a job that runs every sixty seconds → Journal Entry 71

- **DOD-GCP-DOMAIN-1** [trustless-cello, corp-cello-site] — the service is reachable at `api.cello.mygentic.ai` (global LB + managed cert, the pattern `cello-portal` already runs), NOT at a `*.run.app` URL. `DOD-INV-DOMAIN` binds this: a `run.app` hostname in code, copy or configuration is a blocking finding. **`infra/scripts/verify-m11-invariants.sh` is extended to denylist `run.app` and `cloudfunctions.net`** — it currently denylists `execute-api` only, so on GCP it would pass vacuously. Because the hostname is unchanged, item D collapses to a Route 53 record change and corp-cello-site needs no deploy. — 🟠 **PARTIAL.** The enforcer half is ✅ and was the load-bearing one: `verify-m11-invariants.sh` denied only AWS hostnames, so after the port it would have passed while every URL pointed at `*.run.app`. Both Cloud Run hostname shapes and `cloudfunctions.net` are now denied, verified against a probe carrying each; `DOD-INV-DOMAIN` passes. The infrastructure half is ✅: global external ALB at **`35.227.231.107`**, serverless NEG, managed cert, `:80`→`:443`. **Owed, and deliberately not done by an agent: pointing `api.cello.mygentic.ai` at that IP.** That record IS the cutover — it is the moment traffic moves off API Gateway — and this milestone's own ground rule, written the morning of the same day, is that a topology change gets a decision and a checklist rather than arriving as a side effect. The managed certificate stays PROVISIONING until it moves, which is expected rather than a fault → Journal Entry 71

- **DOD-GCP-GATE-1** [trustless-cello] — the ops-agent reaches the waitlist gate over HTTP instead of `lambda:InvokeFunction`. **Fail-closed behaviour is preserved exactly**: a decision is 200 plus a boolean `allowed` and nothing else; any transport failure, non-decision shape, or unparseable body REFUSES rather than admitting, and never reaches the user as "you are not invited". Today's call site (`LambdaWaitlistGateClient`, hardcoded us-east-1) cannot work from Cloud Run — the service has `AWS_DEFAULT_REGION` and `SES_CREDENTIALS` but **no AWS access key**, so the SDK's credential chain finds nothing. It has never failed in production only because it has never been called: zero `waitlist.gate` log lines in 30 days. **A second agent hit this live on 2026-07-31 and shipped the right stopgap** (`36004604`): the gate became an explicit **opt-out**, `WAITLIST_GATE` in `infra/terraform/ops-agent.tf`, currently `disabled` on GCP — because a fail-closed admission check against a hibernated AWS Lambda was blocking the only person who needs to register, for an empty and unlaunched waitlist. The shape is correct: anything but the exact string `disabled` leaves the gate ON, so a missing variable cannot silently admit the world, and it logs `ops_agent.waitlist_gate.disabled` at WARN on every boot. **This line therefore owes one clause more than it did: removing that `WAITLIST_GATE` env from the terraform, so the gate is ON again once it answers on GCP.** Shipping the HTTP client without removing the flag would leave admission permanently open while looking done — a stopgap that outlives its cause is indistinguishable from a hole. — ❌ NOT BUILT

- **DOD-GCP-E2E-1** [all] — the core capture loop, executed against the deployed GCP stack, not locally: real signup → E1 sent through SES → confirm link → session cookie → `/status` renders a real computed queue position and referral link. This is §0a severity 1 and the enforcer for the whole tier. — ❌ NOT BUILT

- **DOD-GCP-OPS-1** [ops-dashboard] — the ops dashboard runs on Cloud Run at `operations.cello.mygentic.ai`, against the GCP portal Cloud SQL. It owns `DOD-INV-WAVE-GATE`, `DOD-WAVE-ASSEMBLY-1` and six `DOD-OPS-*` lines, and it is the only caller of the five direct-invoke handlers — without it the port yields a waitlist nobody can be admitted from. Its four `ops_*` migrations self-apply at boot; its allowlist moves from AWS Secrets Manager `cello/ops/allowed-emails` to GCP Secret Manager. Sequenced AFTER the waitlist port, as its own unit with its own reviewer pass. — ❌ NOT BUILT
...
- **M11-D35 (2026-07-31, Andre) — the waitlist moves to GCP as a P4 tier of M11, keeping the portal database and keeping Lightsail.** M12's cutover left the waitlist as the last AWS-shaped thing; hibernation stops the portal RDS and deletes the NAT, so a full hibernate takes the waitlist with it. Four sub-decisions, taken together because they only make sense together. **(a) The work lands in M11, not M12** — the waitlist is M11's deliverable and this DoD is its sole status authority; M12 owns the cloud, M11 owns the waitlist. **(b) The waitlist keeps living in the portal database.** Andre flagged this as "a little bit hacky — I didn't know how long we'd be using it for", but `DOD-INV-SINGLE-DB` already mandates it in writing, and the GCP portal Cloud SQL is a `db-g1-small` holding 11 migrations and zero rows. It also gives the better exit: when CELLO stops being a waitlist company, the wind-down is dropping 26 migrations' worth of tables out of one instance, not decommissioning a second one. **(c) Lightsail stays as-is, serving the corp site and calling GCP.** It is a static export behind nginx, it is cheap, and cross-cloud is acceptable here because the call is a browser hitting a public API — not a runtime dependency between services. Keeping the hostname `api.cello.mygentic.ai` makes the repoint a DNS record rather than a site deploy. **(d) The ops dashboard comes too** (`DOD-GCP-OPS-1`). The M12 checklist had parked it with "raise it if anyone misses it"; raising it, because it is the only thing that can open a wave, mint a token or credit a post, and §0a ranks admission integrity at severity 2. Reverse: (b) if the waitlist ever needs isolation from portal accounts for a security or compliance reason, split it then — the migrations are self-contained and the ledger keys on stems, so the split is mechanical. (c) if Lightsail's static hosting becomes the thing blocking a dynamic surface, move the site to Cloud Run alongside the API. (d) drop `DOD-GCP-OPS-1` to Parked only if wave admission is genuinely being run by hand against SQL, and say so out loud.
