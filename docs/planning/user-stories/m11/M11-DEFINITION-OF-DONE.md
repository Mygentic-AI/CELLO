...
- **DOD-AUTH-1** — auth + session flows live in corp-cello-site (new routes, same repo and design system — no separate deployment). **Existing:** `app/confirm/ConfirmContent.tsx` already handles the E1 token-verify flow (GET `?token=`, success/expired/invalid states). **P0 remaining work:**
