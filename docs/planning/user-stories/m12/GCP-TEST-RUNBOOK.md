...
export CELLO_DIR=~/.cello-gcp                       # separate profile — a different consortium
                                                    ...
                                                    # rebuild test anyway
export CELLO_DIRECTORY_URL=http://34.75.172.108:8080
...
- **No DNS.** `directory-gcp-*.cello.mygentic.ai` records live in Route53 on hibernated AWS. The
  manifest carries IPs instead; `MULTIADDR-1` made the advertised address configuration precisely so
  this costs nothing.
