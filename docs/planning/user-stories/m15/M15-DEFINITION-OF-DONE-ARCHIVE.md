...
### `DOD-M15-SEALWIRE-1` — the eight bullets, moved 2026-09-02
**Moved out of the scoreboard.** They sat there under a bare `### DOD-M15-SEALWIRE-1 — bullets`
heading with no status tag, one line below the same ID's ✅ pointer — so every scan of open
lines returned it as an eleventh piece of open work. The line closed; these are its clauses.
...
# 📦 SECOND SPLIT — moved 2026-09-04

The scoreboard had grown back to **3,759 lines**, most of it closed work again. Everything below
was moved out of [[M15-DEFINITION-OF-DONE]] on 2026-09-04, **byte-for-byte**, under the same rule
as the 2026-08-24 split: the scoreboard keeps the line and its tag with a one-line pointer here,
and this file keeps the record.

**Three kinds of entry were moved:**

- **CLOSED lines** whose full body was still sitting in the scoreboard.
- **CONTINUATIONS** — live/production verification written into the scoreboard *after* a line's
  body had already been archived on 2026-08-24. Each is filed as `… (continued)` and belongs with
  the original entry of the same ID higher up this file.
- **⬇️ OUT OF GATE lines** — launch does not wait for them, so their bodies are record rather than
  scoreboard. Each keeps its ⬇️ tag and its pointer in the tier it came from.

**One judgement call, flagged rather than buried.** `DOD-M15-SPINERED-1` is still 🟡 **OPEN**, and
the 2026-08-24 rule sends an open line's trail to [[M15-BUILD-JOURNAL]], not here. Its trail is in
this file because what moved is closed triage rather than an investigation still running — and
because this file is the one that was asked for. Move it to the journal if that reading is wrong.
...
### `DOD-M15-SPINERED-1` — 🟡 STILL OPEN in the scoreboard · the CLOSED triage trail, moved 2026-09-04
...
### `DOD-M15-RELAYAUTH-1` — ✅ (continued) the production verification, moved 2026-09-04
...
### `DOD-M15-RELAYADMIN-1` — ✅ (continued) the per-frame correction, moved 2026-09-04
...
### `DOD-M15-RELAYADMIN-DEAD-FRAMES-1` — ✅ (continued) verified in the running container, moved 2026-09-04
...
### `DOD-M15-RELAYABUSE-1` — ✅ (continued) the idle timer read off the running fleet, moved 2026-09-04
...
### `DOD-M15-RELAYSLOTS-1` — ✅ (continued) deployed and attacked in production, moved 2026-09-04
...
### `DOD-M15-SEALROOT-EMPTY-1` — 🅿️ the SUPERSEDED ❌ BLOCKS LAUNCH write-up, moved 2026-09-04
