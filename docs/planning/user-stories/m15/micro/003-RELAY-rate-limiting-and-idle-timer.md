...
enough on its own — a registered agent could still open thousands of sessions with an agent it owns —
so 006 adds slot accounting per agent and per identity pair, a reaper that frees inactive slots before
anyone is refused, and the rule that every refusal reaches the operator with a reason and an
affordance.
