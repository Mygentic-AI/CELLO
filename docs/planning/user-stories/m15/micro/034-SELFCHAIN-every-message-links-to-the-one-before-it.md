---
name: 034-SELFCHAIN — Every message links to the one before it
...
# **<ins>MICRO</ins>** WORK ORDER 034-SELFCHAIN — Everything chains
...
## 🚨 THE ROLLOUT ORDER IS NOT NEGOTIABLE, AND IT HAS A DEPLOY IN THE MIDDLE

`020-ACKHASH` recorded the cost of getting this backwards: a client that emits a layout the deployed
verifiers do not know has **every message refused as `signature_invalid`**, silently, until the
fleet catches up.

- **Part A — READERS.** protocol-types decodes v3; the relay, the directory and the receiving daemon
  all ACCEPT a v3 claim. Nothing emits one. Nothing enforces the new field yet.
- **THE DEPLOY.** npm publish + promote, and the GCP fleet rolled node by node. Part B does not
  start until v3 reading is LIVE ON EVERY NODE — not merely merged.
- **Part B — EMITTER + ENFORCEMENT.** Production signs v3 on every claim. The relay refuses a submit
  whose `prev_own_hash` does not match its record of that sender's previous leaf; the receiving
  daemon runs the same check against its own tree with no relay involved; the directory checks it at
  seal time; the shared seal-chain verifier enforces it on a replayed chain.
- **Part C — DELETE THE WORKAROUND.** With the self-link enforced, `031-RELAYREPLAY`'s
  `seal_chain_sender_clock_reversed` check is dead. Remove it, and remove the comment that describes
...
8. Gate green in both repos. Published, promoted, fleet rolled.
9. Reviewed by `cello-unit-reviewer`, every finding fixed.
