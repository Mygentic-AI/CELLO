...
- `packages/directory/buildspec.yml` — `exported-variables: [IMAGE_URI]` added at top level so `#{BuildAction.IMAGE_URI}` resolves in downstream pipeline actions.
- `packages/relay/buildspec.yml` — same `exported-variables: [IMAGE_URI]` fix.
...
- `packages/e2e-tests/src/__tests__/deploy-005.test.ts` — unit test suite.
