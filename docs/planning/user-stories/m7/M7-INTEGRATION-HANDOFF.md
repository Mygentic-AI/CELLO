...
**STATE ON RESUME: the bounded, low-risk seam work is COMPLETE and paused at a clean milestone.**
Seams 1a–4 prove the entire direct-path session lifecycle in-process at the IPC level (all
reviewed, all green: daemon 342). Every remaining item needs Andre's design/scoping and was
deliberately NOT built blind (see §5 for the why on each):

- **Seam 5 (WIRE-001 initiate restructure)** — needs a DESIGN DECISION: N_A is keyed by
  `session_id`, but the directory generates `session_id`, so "create N_A before negotiate" requires
  a pre-session-id keying / session_offer-round design whose correct shape depends on the WIRE-001
  directory side (which doesn't exist yet). Best done WITH that work, not blind.
- **MSG-001 3b (DAEMON-CONTENT-WIRING)** — the relay content path + recovery; largest remaining
  piece, multi-part native reimplementation; best scoped with Andre. (Task #44.)
- **SESSION-004 legibility, SESSION-002** — greenfield + design-significant; left for Andre.
- **SESSION-003 daemon-liveness** — already covered (verified), no work needed.

So there is no further bounded autonomous seam to take without a design call. The next move is
Andre's: pick seam 5 (with the WIRE-001 directory) or MSG-001 3b (recovery) and scope it.
...
- **Seam 5 — NEEDS A DESIGN DECISION (paused for Andre).** The WIRE-001 initiate restructure is
  NOT a mechanical reorder. `cello_initiate_session` creates N_A keyed by `session_id`, but the
  **`session_id` is generated by the directory/negotiator** — so creating N_A *before* negotiate
  means N_A has no session_id to be keyed by yet. The real protocol resolves this with a
  **session_offer round** (initiator advertises its session node; the directory then returns the
  assignment WITH the session_id). Designing that pre-session-id keying / two-round-offer shape
  correctly depends on the WIRE-001 directory side, **which does not exist yet** — so building it
  blind risks designing the wrong contract. Seam 4 already proves the full direct-path lifecycle
  works (with the test supplying `initiator_session_peer_id` on the push); seam 5 only converts the
  test's two-phase orchestration into the single-round production negotiator shape. **Decision for
  Andre:** do seam 5 alongside the WIRE-001 directory work so the contract is validated against its
  real consumer, OR confirm the offer-round design up front.

**Remaining work — each needs Andre's scoping/design (greenfield or large), so paused here:**
- **MSG-001 3b** (`CELLO-M7-DAEMON-CONTENT-WIRING`) — the daemon **relay content-leaf path** +
  recovery (Structure2 hash-leaf submit/receive + relay-assigned sequence + recovery
  sender_resend/relay_queue + production park deposit). The spine is **BUILT AND TESTED on the dead
  `core/client` stack** (`relay-stream-manager.ts` 1261 lines + `session-manager.ts`); it must be
  REIMPLEMENTED natively under Option A. **Largest remaining piece** (the recovery half of M6B
  parity) and a multi-part effort — best scoped with Andre. (Task #44.)
- **SESSION-003** — daemon-liveness: **DONE / already covered.** Verified `session-003-direct-liveness.test.ts`
  (2 tests, real libp2p) exercises the daemon `#sessionLiveness`/`#wireSessionLiveness` path —
  'alive' on connect, 'gone' on real disconnect (WARN), 'unknown' untracked. The earlier "untested"
  note was stale. (A separate consumer — a seal-gate that READS liveness — was in the dead
  `seal-manager`; that belongs to the seal-flow re-home, not a liveness gap.)
- **SESSION-004** — legibility: **GREENFIELD, not a re-home.** Verified: no `legibility` construction
  exists anywhere in `core/client` (only the protocol-types schema is live). The story is `status:
  draft`, P1, spanning directory (builds the cert) + client (renders/verifies) + the seal flow —
  design-significant greenfield. Left for Andre. (Also where the deferred inbound-assignment FROST
  verification + the single-key refusal context belong.)
- **SESSION-002** — CONFIRMED greenfield (only the YAML exists). Client/daemon half from scratch.
  Left for Andre.
