...
| 12 | Any-directory routing | A | ✅ VERIFIED (via FINDING-4 failover, 2026-07-03) | The "needs manual reconnect" blocker is SUPERSEDED — directory selection is AUTOMATIC (roster-aware failover). Killed us1 → client resolved + operated against **eu1** `verified:true`; killed eu1 → **ap1** `verified:true` (step-6 identity-checked). Client runs fully against a non-home directory. See STATE.md 2026-07-03 reconciliation. |
| 13 | Cross-node presence | A | ✅ COVERED (FINDING-4 failover, 2026-07-03) | During failover the client ran signaling/relay/ceremony resolution against non-home eu1/ap1 (`signaling.connected verified:true`), which exercises the cross-node presence path. A dedicated "presence query from a non-home directory" probe was not separately logged. |
...
| 9 | Node down during DKG | C | ✅ ADDRESSED + SPINE-VERIFIED (M8B Problem 2, 2026-07-04) | Root gap was **all-N registration** (any directory down → registration refused). Fixed: register among the AVAILABLE quorum Q (client sends reachable node list R; directory picks Q = R ∩ manifest, requires Q ≥ t). Spine GREEN (`j-tofn-dkg`, real 3-node binaries): kill 1 of 3 → registers among the 2-node quorum, threshold 2. Deployed all 3 regions (`cello-directory:bb02899`); client beta `v0.0.69` (daemon 0.0.28). **Not yet** live-AWS demoed (dev N=3 needs t=2, or a cluster N>3 — optional follow-up, not a blocker). Plan: [[2026-07-04_0556_tofn-registration-availability-quorum-enrollment-plan]]. |
| 10 | Node down during seal | C | ⚠️ CODE-ADDRESSED, not live-verified | Seal now TARGETS the recorded quorum Q (persist-Q fix: `frost_directory_node_ids` persisted + `hydrateShareAndStubs` filters the roster to Q), so a restored signer aims at holders, not the full roster. Survive-a-node-down at seal is DOD-SIGN-1 (spine-proven `j-sign`) + the coordinator's exclude-and-retry. A live/spine seal by a **quorum-registered** agent WITH a directory down is still to run. Both code-review Criticals here fixed (persist-Q, refresh-forward); 523 daemon + 673 directory tests green. |
| 5 | Directory reconnect | C | ✅ VERIFIED (FINDING-4 failover, 2026-07-03) | Killing the primary directory → client reconnected to a reachable roster member automatically (roster-aware resolver, `directory.bootstrap.failover`). Survives 2 of 3 directories down. |
| 4 | Relay failover | E | ⬜ pending — DISTINCT from directory failover | Directory failover (✅ #12/#5) carries the client onto the fallback region's relay implicitly. A dedicated relay-down-but-directory-up case (relay is per-region, not global) is NOT yet run. |
...
- [[2026-07-04_1730_cross-node-session-topology|Cross-node session topology]] — the design that closes FINDING-9's cross-node routing gap and elevates FINDING-8's boot-only profile cache to a broker-side dependency check; its acceptance scenario 1 is the FINDING-9 topology re-run.
