...
| | _Partial: the dependency-free steps (strip / confusables / decode / entropy / special-token / size cap) are built + unit-tested. AC-002 (RE2 injection-pattern match) is PARKED on the RE2-binding decision; screen-fn wiring lands with M9-IN-002._ | | |
...
| | _Decision-coupled: the comprehensive 222-rule gitleaks set is RE2-authored — waits on the RE2-binding decision (+ the rule-port). Deferred with the inbound detectors._ | | |
