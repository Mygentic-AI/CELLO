...
| Firewall | `cello-directory-allow-iap-ssh` (35.235.240.0/20 → 22), `cello-directory-allow-protocol` (0.0.0.0/0 → 4000, 8080), `cello-directory-allow-http` (0.0.0.0/0 → 9090) | Protocol ports are public **on purpose**: libp2p is Noise-encrypted and manifest-pinned, so the transport authenticates its own peers — that is what lets directories reconcile across clouds with no tunnel. **9090 IS PUBLIC and has to be** — it serves `GET /bootstrap`, which is how a client learns this node's multiaddr and peer id; without it a client resolves ZERO nodes from a perfectly valid manifest. `/health` rides the same port and returns nodeId + schemaVersion, both already public via the signed manifest. ⚠️ **This row previously read "the health port is unauthenticated and is NOT public" and named a `cello-directory-allow-health-probes` rule.** Corrected 2026-09-06 against `terraform/node-directory.tf`: that rule is the RELAY's, Google's prober ranges are covered by 0.0.0.0/0 and no longer need their own rule, and the client's plaintext bootstrap fetch depends on this port being open. Path-selective exposure returns with a TLS terminator — see `DOD-M15-BOOTSTRAP-TLS-1`. |
