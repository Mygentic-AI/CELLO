...
| @cello-protocol versions | **daemon 0.0.20 / cli 0.0.18 / connect 0.0.53** (+ crypto 0.0.14, transport 0.0.11, client 0.0.41, protocol-types 0.0.11) — updated 2026-07-02 (was daemon 0.0.13/cli 0.0.11/connect 0.0.49 on 2026-06-27; note the 0.0.19 interim on 2026-06-30 for M8B was never reflected here) |
...
| Service status | **UPDATED 2026-07-02 to daemon 0.0.20 (Finding 1 + F2-a fixes)** — `cd /opt/cello-demo && npm install @cello-protocol/cli@0.0.18` (caret `^0.0.17` pins exactly for 0.0.x, so the explicit bump was required to pull daemon 0.0.20). No DB migration (the fixes add an in-memory map + methods only; FROST share untouched, no re-registration). Restarted daemon→demo in sequence; both active; healthy startup (`agent.signaling.created`→`directory.signaling.connected`→`agent.online`→`session.node.created`), standing receiver created, `demo.started`. One transient `directory.signaling.reader.error: signaling_closed` warn during startup (reconnected — 2 connects), non-fatal. Prior 2026-06-27 note: 0.0.13 migrated flat-file→SQLCipher cleanly (backup at `/opt/cello-demo/.cello.bak-pre-0013`). |
