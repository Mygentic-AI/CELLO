...
# One JSON blob, the same shape as the AWS RDS secret ({username, password, host, port, dbname}),
# so gcp-boot-env and the AWS entrypoint agree on what a database credential looks like. `host` is
# the PSC forwarding-rule address — the only route to this instance that exists.
