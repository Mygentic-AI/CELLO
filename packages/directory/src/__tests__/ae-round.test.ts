...
describe("DOD-AE-APPEND-1: planRound", () => {
  it("pulls Tier-A record hashes the local node lacks, per table", async () => {
    const local: LocalRoundState = {
      tierA: new Map([["agent_profiles", [h(1), h(2)]]]),
      tierB: new Map(),
    };
    const peer: PeerRoundState = {
      tierA: new Map([["agent_profiles", { digest: "x", recordHashes: async () => [h(1), h(2), h(3)] }]]),
      tierB: new Map(),
    };
    const plan = await planRound(local, peer);
    expect(plan.tierA.get("agent_profiles")).toEqual([h(3)]);
  });

  it("a converged Tier-A table (matching digest) produces no pull", async () => {
    // NB: the digest-match `continue` is a compute/wire optimization with NO observable difference
    // from the drill-down path (on a match the peer sends recordHashes: async () => [], so missingLocally→[]);
    // no test can isolate the skip itself. This pins the outcome.
    const same = [h(1), h(2)];
    const local: LocalRoundState = { tierA: new Map([["t", same]]), tierB: new Map() };
    const peer: PeerRoundState = {
      tierA: new Map([["t", { digest: computeTableDigest(same), recordHashes: async () => [] /* not sent when digest matches */ }]]),
      tierB: new Map(),
    };
    const plan = await planRound(local, peer);
  ...
  it("pulls Tier-B keys whose version differs or the peer holds and local lacks", async () => {
    const local: LocalRoundState = {
      tierA: new Map(),
    ...
    const peer: PeerRoundState = {
      tierA: new Map(),
      tierB: new Map([["agent_suspensions", { digest: "y", versions: async () => new Map([["ag1", "v1"], ["ag2", "vCHANGED"], ["ag3", "v9"]])}]]),
    };
    const plan = await planRound(local, peer);
    expect(new Set(plan.tierB.get("agent_suspensions"))).toEqual(new Set(["ag2", "ag3"]));
  });

  it("differing digest but peer is a SUBSET → nothing to pull, table absent from plan (F2 guard)", async () => {
    ...
    const peer: PeerRoundState = {
      tierA: new Map([["t", { digest: "differs", recordHashes: async () => [h(1)] }]]),
      tierB: new Map([["u", { digest: "differs", versions: async () => new Map([["k1", "v1"]])}]]),
    };
    const plan = await planRound(local, peer);
  ...
  it("an untracked table advertised EMPTY → digests match → skip (F3)", async () => {
    const local: LocalRoundState = { tierA: new Map(), tierB: new Map() };
    const peer: PeerRoundState = {
      tierA: new Map([["t", { digest: computeTableDigest([]), recordHashes: async () => [] }]]),
      tierB: new Map([["u", { digest: tierBTableDigest(new Map()), versions: async () => new Map()}]]),
    };
    const plan = await planRound(local, peer);
    expect(plan.tierA.has("t")).toBe(false);
    expect(plan.tierB.has("u")).toBe(false);
  ...
  it("skips a Tier-B table whose digest matches", async () => {
    ...
    const peer: PeerRoundState = {
      tierA: new Map(),
      tierB: new Map([["agent_presence", { digest: tierBTableDigest(versions), versions: async () => new Map()}]]),
    };
    expect((await planRound(local, peer)).tierB.has("agent_presence")).toBe(false);
  });

  it("a table the peer advertises but the local node doesn't track is pulled wholesale (Tier-A)", async () => {
    const local: LocalRoundState = { tierA: new Map(), tierB: new Map() };
    const peer: PeerRoundState = {
      tierA: new Map([["seal_notarizations", { digest: "z", recordHashes: async () => [h(5), h(6)] }]]),
      tierB: new Map(),
    };
    expect(new Set((await planRound(local, peer)).tierA.get("seal_notarizations"))).toEqual(new Set([h(5), h(6)]));
  });

  it("nothing to pull when everything matches → empty plan", async () => {
    const a = [h(1)];
    const v = new Map([["k", "v1"]]);
    const local: LocalRoundState = { tierA: new Map([["t", a]]), tierB: new Map([["u", v]]) };
    const peer: PeerRoundState = {
      tierA: new Map([["t", { digest: computeTableDigest(a), recordHashes: async () => [] }]]),
      tierB: new Map([["u", { digest: tierBTableDigest(v), versions: async () => new Map()}]]),
    };
    const plan = await planRound(local, peer);
