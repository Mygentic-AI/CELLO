...
export class CelloDirectoryNode {
  ...
  async #processRegisterRequest(
    ...
    // M12-D8: the node set comes from the VERIFIED manifest, never the served copy — the quorum and
    // threshold are derived from it, so a tampered manifest would steer which nodes hold shares and
    // what T is. (Pre-M12 this read the unverified transport copy; the role split closes that gap.)
    const allManifestNodes = this.#directoryManifestStore?.getVerifiedManifest()?.nodes ?? [];
  ...
  async #processSealUnilateral(
    ...
    // adds the teeth that replace the relay's authoritative log: per own-party leaf the relay receipt
    ...
    const leafData: RelaySealData = { leaves: carried.leaves, seq_count: carried.leaves.length, merkle_root: new Uint8Array(32) };
    this.#logger?.info("session.unilateral.leaves.rebuilt", {
      sessionId: sessionIdHex,
      leafCount: leafData.leaves.length,
      source: "client_carried",
      correlationId,
    });
