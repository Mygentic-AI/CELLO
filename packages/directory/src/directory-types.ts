 ...
 */
// 031-RELAYREPLAY: DEFINED IN `@cello-protocol/interfaces`, not here. The relay verifies a replayed
// chain of these when it inherits a conversation, so the shape now lives in the one package both
// nodes import. Re-exported so every existing importer of `directory-types.js` is unchanged.
import type { SealUnilateralLeaf } from "@cello-protocol/interfaces";
export type { SealUnilateralLeaf };
...
// ─── Relay seal data ──────────────────────────────────────────────────────────
//
// 031-RELAYREPLAY: DEFINED IN `@cello-protocol/interfaces`. This block used to say *"mirror of
// relay-types SealData, kept local to avoid cross-package import"* — which is exactly the duplication
// this milestone has now paid for twice (017-TBS, 020-ACKHASH). The shared workspace package is what
// makes the cross-package import cheap, so the mirror is gone and both nodes read one definition.
// Re-exported so every existing importer of `directory-types.js` is unchanged.
export type { RelaySealLeafKind, RelaySealLeaf, RelaySealData } from "@cello-protocol/interfaces";
