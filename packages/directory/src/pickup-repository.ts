/**
 * Pickup queue drain + ACK (the daemon-delivery half of the trust-signal pipe).
 *
 * On signaling reconnect the directory drains an agent's unacked sealed signals, delivers each over
 * the stream, and on the daemon's ACK DELETES the row — so after ACK the queue holds NO ciphertext
 * for that signal. Each delivered item carries its signal_hash directly on the row (M10-D22).
 */
...
/** Drain an agent's unacked sealed signals, oldest first. */
...
export async function ackPickupDelete(db: Queryable, id: string, agentId: string): Promise<void> {
  // ACCOUNT-SCOPED: the delete is bound to the ACK'ing agent's own agent_id, NOT id alone.
...
/**
 * Sweep orphaned pickups: pending rows older than ttlHours that have no signal_hash (legacy M8
 * leftovers that can never be delivered — M10 rows always carry their own hash). Scoped to this
 * node's own rows for replication safety.
 */
