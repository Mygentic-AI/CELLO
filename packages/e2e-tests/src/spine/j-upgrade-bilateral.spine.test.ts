...
describe("J-UPGRADE-001 — unilateral → bilateral upgrade on the absent party's return (DOD-UP-1)", () => {
  it("B killed → A seals UNILATERAL → B returns, recovers+verifies, RATIFIES → directory records a superseding BILATERAL notarization over the same root", async () => {
    ...
    expect(((await connA.call("cello_send", { session_id: sessionIdA, content: "upgrade-to-bilateral message" })) as { ok?: boolean }).ok).toBe(true);
    expect(((await connB.call("cello_receive", { session_id: sessionIdB, timeout_ms: 15_000 })) as { content?: string | null }).content).toBe("upgrade-to-bilateral message");
