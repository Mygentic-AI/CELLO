...
/**
 * ⚠️ EVERY SUBMIT BELOW PASSES `prevOwnHash: genesis` — `DOD-M15-SELFCHAIN-1`.
 *
 * Both chain links are required now, and the relay checks both. Each test here builds its own fresh
 * harness and each party sends at most one leaf, so every submit in this file is that author's FIRST
 * — including the counter-submits, where the leaf is the AUTHOR's first even though a different
 * party hands it over. The honest self link for a first message is the session genesis.
 *
 * A test that added a second leaf from the same author would have to name that author's first
 * content hash instead; the required field is what would surface it, as a type error.
 */
...
describe("034-CARRYLEAF: a withheld message can be witnessed by the party who received it", () => {
  ...
  it("★★ B witnesses a leaf A AUTHORED and never submitted — the withholding attack, closed", async () => {
    ...
    const first = await h.submit({ contentHash: new Uint8Array(randomBytes(32)), lastSeenSeq: 0, timestamp: Date.now(), lastSeenHash: genesis, prevOwnHash: genesis });
    ...
    const rescued = await h.submitAsB({
      ...
      lastSeenSeq: 0,
      timestamp: Date.now(),
      lastSeenHash: genesis, prevOwnHash: genesis,
      authorKp: h.clientA,
    });
  ...
  it("★★ a counter-submit may NOT replay a leaf the relay already holds", async () => {
    ...
    const honest = await h.submit({ contentHash: content, lastSeenSeq: 0, timestamp: honestTimestamp, lastSeenHash: genesis, prevOwnHash: genesis });
  ...
  it("★★ BOTH PARTICIPANTS ARE TOLD — a counter-submit is reported, not just logged", async () => {
    ...
    await h.submit({ contentHash: new Uint8Array(randomBytes(32)), lastSeenSeq: 0, timestamp: Date.now(), lastSeenHash: genesis, prevOwnHash: genesis });
