...
async function performRelayAuth(reader: StreamReader, stream: Stream, kp: ReturnType<typeof generateKeypair>, dirKp: ReturnType<typeof generateKeypair>): Promise<void> {
...
async function sealWithBroker(opts: {
  ...
  for (const [kp, seq] of [[clientA, 0], [clientB, 1]] as const) {
    ...
    await performRelayAuth(reader, stream, kp, configuredDir);
